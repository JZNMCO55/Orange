...
#define ORANGE_KEY_CASE(N)                                        \
    if (IEquals(namePart, #N))                                    \
    {                                                             \
        out.codeOrButton = static_cast<std::int32_t>(KeyCode::N); \
        return true;                                              \
    }
                ...
                ORANGE_KEY_CASE(A)
                ORANGE_KEY_CASE(B) ORANGE_KEY_CASE(C) ORANGE_KEY_CASE(D)
...
#undef ORANGE_KEY_CASE
            ...
            if (IEquals(srcPart, "mouse"))
            {
                ...
                if (IEquals(namePart, "Left"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(MouseButton::Left);
                    return true;
                }
                if (IEquals(namePart, "Right"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(MouseButton::Right);
                    return true;
                }
                if (IEquals(namePart, "Middle"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(MouseButton::Middle);
                    return true;
                }
                return false;
            }
            if (IEquals(srcPart, "gamepad"))
            {
                ...
                if (IEquals(namePart, "South"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::South);
                    return true;
                }
                if (IEquals(namePart, "East"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::East);
                    return true;
                }
                if (IEquals(namePart, "West"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::West);
                    return true;
                }
                if (IEquals(namePart, "North"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::North);
                    return true;
                }
                if (IEquals(namePart, "LB"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::LB);
                    return true;
                }
                if (IEquals(namePart, "RB"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::RB);
                    return true;
                }
                if (IEquals(namePart, "Back"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::Back);
                    return true;
                }
                if (IEquals(namePart, "Start"))
                {
                    out.codeOrButton = static_cast<std::int32_t>(GamepadButton::Start);
                    return true;
                }
                return false;
            }
