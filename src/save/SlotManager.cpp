...
{
    if (!IsSafeSlotName(slotName)) { return false; }
    auto dirResult = ResolveSaveDirectory(mOptions);
    if (dirResult.IsErr()) { return false; }
                  ...
                  SlotMetadata          metadata) const
{
    ...
    if (savePathResult.IsErr()) { return savePathResult.Error(); }
    auto metaPathResult = ResolveMetadataPath(slotName);
    if (metaPathResult.IsErr()) { return metaPathResult.Error(); }
...
SlotManager::ListSlots() const
{
    ...
    {
        std::error_code itEc;
        if (!entry.is_regular_file(itEc) || itEc) { continue; }

        const auto& path = entry.path();
        if (path.extension() != saveExt) { continue; }   // 跳过 .meta.json / .tmp / 其它文件
