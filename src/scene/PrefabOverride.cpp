...
namespace Orange::Engine::Scene
{
    namespace
    {
        ...
        {
            return componentName == "Guid" || componentName == "PrefabInstance" || componentName == "Hierarchy";
        }
    ...
    {
        if (!instWorld.IsValid(instEntity) || !baseWorld.IsValid(baseEntity) || !newWorld.IsValid(newEntity))
        {
            return false;
        }
                                     ...
                                     const Asset::PrefabAsset& newTmpl)
    {
        ...
        World baseWorld;
        World newWorld;
        if (Scene::LoadFromString(baseTmpl.TemplateBlob(), baseWorld).IsErr() || Scene::LoadFromString(newTmpl.TemplateBlob(), newWorld).IsErr())
        {
            return false;
        }
                            ...
                            std::string_view         fieldPath)
    {
        ...
        // dedup：已存在则不重复加。
        if (std::find(link.overriddenPaths.begin(), link.overriddenPaths.end(), path) != link.overriddenPaths.end())
        {
            return false;
        }
    ...
    {
        const std::string path = MakeOverridePath(componentName, fieldPath);
        return std::find(link.overriddenPaths.begin(), link.overriddenPaths.end(), path) != link.overriddenPaths.end();
    }
                                  ...
                                  std::string_view componentName, std::string_view fieldPath)
    {
        ...
        const bool componentPresent =
            std::find(componentNames.begin(), componentNames.end(), std::string(componentName)) != componentNames.end();
        bool writeOk = true;
        if (componentPresent)
        {
