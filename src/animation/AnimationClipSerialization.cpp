...
namespace Orange::Engine::Animation
{
        ...
        AnimationClip ParseClipPayload(const JsonReader& reader)
        {
            ...
            for (std::size_t i = 0; i < trackCount; ++i)
            {
                ...
                if (reader.ReadString(Join(tp, "valueType"), vtStr))
                {
                    ...
                    if (TrackValueTypeFromString(vtStr, vt))
                    {
                        track.valueType = vt;
                    }
                }
                ...
                for (std::size_t j = 0; j < keyCount; ++j)
                {
                    ...
                    if (reader.ReadString(Join(kp, "interp"), interpStr))
                    {
                        ...
                        if (InterpModeFromString(interpStr, mode))
                        {
                            key.interp = mode;
                        }
                    }
    ...
    bool TrackValueTypeFromString(std::string_view text, TrackValueType& out) noexcept
    {
        if (text == "Float")
        {
            out = TrackValueType::Float;
            return true;
        }
        if (text == "Vec2")
        {
            out = TrackValueType::Vec2;
            return true;
        }
        if (text == "Vec3")
        {
            out = TrackValueType::Vec3;
            return true;
        }
        if (text == "Vec4")
        {
            out = TrackValueType::Vec4;
            return true;
        }
        if (text == "Quat")
        {
            out = TrackValueType::Quat;
            return true;
        }
        return false;
    }
    ...
    bool InterpModeFromString(std::string_view text, InterpMode& out) noexcept
    {
        ...
        if (text == "Linear")
        {
            out = InterpMode::Linear;
            return true;
        }
        if (text == "Bezier")
        {
            out = InterpMode::Bezier;
            return true;
        }
        return false;
    }
