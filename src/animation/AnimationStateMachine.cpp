...
bool EvaluateExpr(const ConditionExpr& expr, const Parameter& param) noexcept
{
    switch (expr.op)
    {
        case ConditionOp::If:           return AsBool(param.value);
        case ConditionOp::IfNot:        return !AsBool(param.value);
        case ConditionOp::Greater:      return AsFloat(param.value)  >  AsFloat(expr.threshold);
        case ConditionOp::Less:         return AsFloat(param.value)  <  AsFloat(expr.threshold);
        case ConditionOp::Equal:        return AsFloat(param.value) ==  AsFloat(expr.threshold);
        case ConditionOp::NotEqual:     return AsFloat(param.value) !=  AsFloat(expr.threshold);
        case ConditionOp::GreaterEqual: return AsFloat(param.value)  >= AsFloat(expr.threshold);
        case ConditionOp::LessEqual:    return AsFloat(param.value)  <= AsFloat(expr.threshold);
    }
    return false;
}
...
bool AnimationStateMachine::GetParameterBool(std::string_view name) const noexcept
{
    ...
    auto it = mpImpl->parameters.find(std::string(name));
    if (it == mpImpl->parameters.end()) { return false; }
...
std::int32_t AnimationStateMachine::GetParameterInt(std::string_view name) const noexcept
{
    if (!mpImpl) { return 0; }
    auto it = mpImpl->parameters.find(std::string(name));
    if (it == mpImpl->parameters.end()) { return 0; }
...
{
    if (!mpImpl) { return 0.0f; }
    auto it = mpImpl->parameters.find(std::string(name));
    if (it == mpImpl->parameters.end()) { return 0.0f; }
