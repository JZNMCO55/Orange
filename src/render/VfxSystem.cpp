...
struct VfxSystem::Impl
{
    ...
    bool CreateInstanceBuffer()
    {
        ...
        const std::uint64_t total = kSliceBytesPerSlot
                                  * static_cast<std::uint64_t>(framesInFlight);

        Orange::Rhi::BufferDesc desc{};
        desc.mSize        = total;
        desc.mUsage       = Orange::Rhi::BufferUsage::Vertex
...
void VfxSystem::Tick(const World& world, float dt)
{
    ...
    for (auto it = impl.pools.begin(); it != impl.pools.end(); )
    {
        if (!world.IsValid(it->first)
            || !world.HasComponent<ParticleEmitterComponent>(it->first))
        {
...
{
    auto& impl = *mpImpl;
    if (!impl.initialized || pCmdList == nullptr || pHdrColorView == nullptr
        || viewProjMatrixData == nullptr)
    {
        return;
    }
    if (impl.pools.empty())
    ...
    push.timeAspect = glm::vec4(0.0f, 0.0f,
                                hdrHeight > 0 ? static_cast<float>(hdrWidth)
                                              / static_cast<float>(hdrHeight)
                                              : 1.0f,
                                0.0f);
