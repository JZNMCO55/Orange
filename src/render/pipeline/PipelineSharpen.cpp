...
bool Pipeline::Impl::EnsureSharpenResources()
{
    if (renderDevice == nullptr || hdrColor == nullptr || hdrSampler == nullptr
        || sharpenPipeline == nullptr || dofCompositePipeline == nullptr
        || sharpenLayout == nullptr || bloomLayout == nullptr || sharpenUbo == nullptr)
    {
        return false;
    }
    ...
    if (sharpenCompositeSet == nullptr)
    {
        ...
        if (!sharpenCompositeSet) { return false; }
        sharpenCompositeSetBound = nullptr;
    }
...
bool Pipeline::Impl::RecordSharpenPass(const SharpenPass& sharpenDesc)
{
    ...
    // ---- Pass 1：gather（hdrColor[ShaderReadOnly] 邻域 → sharpColor）----
    {
        ...
        sc.mWidth = sharpenColorWidth; sc.mHeight = sharpenColorHeight;
        cmd.SetScissor(sc);
