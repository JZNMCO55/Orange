...
namespace Orange::Engine::Render
{
    ...
    {
        if (renderDevice == nullptr || sceneDepth == nullptr || hdrColor == nullptr || hdrSampler == nullptr || taaResolvePipeline == nullptr || dofCompositePipeline == nullptr || taaLayout == nullptr || bloomLayout == nullptr || taaUbo == nullptr)
        {
            return false;
        }
                                          ...
                                          const glm::mat4& curJitteredViewProj)
    {
        ...
        // ---- Pass 1：resolve（hdrColor + prevHistory + depth → curHistory）----
        {
            ...
            sc.mWidth  = taaHistoryWidth;
            sc.mHeight = taaHistoryHeight;
            cmd.SetScissor(sc);
