...
namespace Orange::Engine::Render
{
    ...
    {
        if (renderDevice == nullptr || hdrColor == nullptr || hdrSampler == nullptr || colorGradePipeline == nullptr || dofCompositePipeline == nullptr || colorGradeLayout == nullptr || bloomLayout == nullptr || colorGradeUbo == nullptr)
        {
            return false;
        }
    ...
    bool Pipeline::Impl::RecordColorGradePass(const ColorGradePass& gradeDesc)
    {
        ...
        // ---- Pass 1：grade（hdrColor[ShaderReadOnly] → gradeColor）----
        {
            ...
            sc.mWidth  = gradeColorWidth;
            sc.mHeight = gradeColorHeight;
            cmd.SetScissor(sc);
