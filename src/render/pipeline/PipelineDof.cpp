...
bool Pipeline::Impl::EnsureDofResources()
{
    if (renderDevice == nullptr || sceneDepth == nullptr || hdrColor == nullptr
        || hdrSampler == nullptr || dofPipeline == nullptr
        || dofCompositePipeline == nullptr || dofLayout == nullptr
        || bloomLayout == nullptr || dofUbo == nullptr)
    {
        return false;
    }
...
bool Pipeline::Impl::RecordDofPass(const DofPass& dofDesc, const glm::mat4& proj)
{
    ...
    // ---- Pass 1：dof gather（采 hdrColor[ShaderReadOnly] + depth → dofColor）----
    {
        ...
        sc.mWidth = dofColorWidth; sc.mHeight = dofColorHeight;
        cmd.SetScissor(sc);
