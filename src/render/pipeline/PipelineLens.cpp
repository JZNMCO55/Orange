...
bool Pipeline::Impl::EnsureLensResources()
{
    if (renderDevice == nullptr || hdrColor == nullptr || hdrSampler == nullptr
        || lensPipeline == nullptr || dofCompositePipeline == nullptr
        || lensLayout == nullptr || bloomLayout == nullptr || lensUbo == nullptr)
    {
        return false;
    }
    ...
    if (lensCompositeSet == nullptr)
    {
        ...
        if (!lensCompositeSet) { return false; }
        lensCompositeSetBound = nullptr;
    }
...
bool Pipeline::Impl::RecordLensPass(const LensPass& lensDesc)
{
    ...
    // ---- Pass 1：gather（hdrColor[ShaderReadOnly] → lensColor）----
    {
        ...
        sc.mWidth = lensColorWidth; sc.mHeight = lensColorHeight;
        cmd.SetScissor(sc);
