...
namespace Orange::Engine::Render
{
    ...
    bool Pipeline::Impl::EnsureShadowMap()
    {
        ...
        for (auto& v : shadowMapLayerViews)
        {
            v.reset();
        }
        shadowMap.reset();
        ...
        t.mUsage       = Orange::Rhi::TextureUsage::DepthStencil | Orange::Rhi::TextureUsage::Sampled;
    ...
    bool Pipeline::Impl::EnsureSpotShadowArray()
    {
        ...
        renderDevice->WaitIdle();
        for (auto& v : spotShadowLayerViews)
        {
            v.reset();
        }
    ...
    bool Pipeline::Impl::EnsurePointShadowCube()
    {
        ...
        renderDevice->WaitIdle();
        for (auto& v : pointShadowFaceViews)
        {
            v.reset();
        }
        for (auto& c : pointShadowCubes)
        {
            c.reset();
        }
