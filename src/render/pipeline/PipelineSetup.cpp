...
namespace Orange::Engine::Render
{
    ...
    Result<void, ResultCode> Pipeline::SetupRhiResources()
    {
        ...
        layoutDesc.mBindings.push_back({/*mBinding=*/0,
        ...
        }
        {
            ...
            if (downCode.empty() || upCode.empty() || combineCode.empty() || tonemapVsCode.empty() || tonemapFsCode.empty() || godRaysCode.empty() || skyCode.empty() || proceduralSkyCode.empty())
            {
                Shutdown();
                return ResultCode::IoError;
            }
            ...
            if (!impl.bloomDownsampleFs || !impl.bloomUpsampleFs || !impl.passthroughCombineFs || !impl.tonemapVs || !impl.tonemapFs || !impl.godRaysFs || !impl.skyFs || !impl.proceduralSkyFs)
            {
        ...
        }
        {
            ...
            if (!impl.ssaoFs || !impl.ssaoApplyFs || !impl.gtaoFs || !impl.ssaoLayout || !impl.ssaoUbo)
            {
            ...
            for (std::uint32_t i = 0; i < Pipeline::Impl::kSsaoKernelSize; ++i)
            {
                ...
                const float t     = static_cast<float>(i) / static_cast<float>(Pipeline::Impl::kSsaoKernelSize);
        ...
        {
            // 相机运动模糊 gather pipeline —— hdrColor + depth + ubo → motionBlurColor
            // （RGBA16F，no blend）。composite 复用 dofCompositePipeline。
            auto mbCode = LoadSpirv("shaders/orange_engine/motion_blur.frag.spv");
            if (mbCode.empty())
            {
                ORANGE_LOG_ERROR("Pipeline::Initialize: motion_blur shader .spv 加载失败");
                Shutdown();
                return ResultCode::InternalError;
            }
            Orange::Rhi::ShaderModuleDesc sm{};
            sm.mStage         = Orange::Rhi::ShaderStage::Fragment;
            sm.mpCode         = mbCode.data();
            sm.mCodeSize      = mbCode.size() * sizeof(std::uint32_t);
            sm.mpDebugName    = "orange_engine.motion_blur.frag";
            impl.motionBlurFs = rhi.CreateShaderModule(sm);

            // motionBlurLayout：0 = hdrColor，1 = MotionBlurUbo，2 = sceneDepth。
            Orange::Rhi::DescriptorSetLayoutDesc lay{};
            lay.mBindings.push_back({0, Orange::Rhi::DescriptorType::CombinedImageSampler,
                                     1, Orange::Rhi::ShaderStage::Fragment});
            lay.mBindings.push_back({1, Orange::Rhi::DescriptorType::UniformBuffer,
            ...
            lay.mBindings.push_back({2, Orange::Rhi::DescriptorType::CombinedImageSampler,
                                     1, Orange::Rhi::ShaderStage::Fragment});
            lay.mpDebugName       = "orange_engine.motion_blur.layout";
            impl.motionBlurLayout = rhi.CreateDescriptorSetLayout(lay);

            Orange::Rhi::BufferDesc ub{};
            ub.mSize           = sizeof(Pipeline::Impl::MotionBlurUboData);
            ub.mUsage          = Orange::Rhi::BufferUsage::Uniform;
            ub.mMemoryUsage    = Orange::Rhi::MemoryUsage::CpuToGpu;
            impl.motionBlurUbo = rhi.CreateBuffer(ub);

            if (!impl.motionBlurFs || !impl.motionBlurLayout || !impl.motionBlurUbo)
            {
                ORANGE_LOG_ERROR("Pipeline::Initialize: motion blur 资源创建失败");
                Shutdown();
                return ResultCode::InternalError;
            }
            ...
            d.mColorBlend.mAttachments.push_back({}); // no blend
            d.mRenderTargets.mColorFormats.push_back(kHdrColorFormat);
            d.mDescriptorSetLayouts.push_back(impl.motionBlurLayout.get());
            d.mpDebugName           = "orange_engine.motion_blur";
            impl.motionBlurPipeline = rhi.CreateGraphicsPipeline(d);
            if (!impl.motionBlurPipeline)
            {
                ORANGE_LOG_ERROR("Pipeline::Initialize: motion blur pipeline 创建失败");
                Shutdown();
                return ResultCode::InternalError;
            }
        }
        {
            // 镜头效果（色散 + 暗角）gather pipeline —— hdrColor + ubo → lensColor
            // （RGBA16F，no blend）。composite 复用 dofCompositePipeline。
            auto lensCode = LoadSpirv("shaders/orange_engine/lens.frag.spv");
            if (lensCode.empty())
            {
                ORANGE_LOG_ERROR("Pipeline::Initialize: lens shader .spv 加载失败");
                Shutdown();
                return ResultCode::InternalError;
            }
            Orange::Rhi::ShaderModuleDesc sm{};
            sm.mStage      = Orange::Rhi::ShaderStage::Fragment;
            sm.mpCode      = lensCode.data();
            sm.mCodeSize   = lensCode.size() * sizeof(std::uint32_t);
            sm.mpDebugName = "orange_engine.lens.frag";
            impl.lensFs    = rhi.CreateShaderModule(sm);

            // lensLayout：0 = hdrColor，1 = LensUbo。
            Orange::Rhi::DescriptorSetLayoutDesc lay{};
            lay.mBindings.push_back({0, Orange::Rhi::DescriptorType::CombinedImageSampler,
                                     1, Orange::Rhi::ShaderStage::Fragment});
            lay.mBindings.push_back({1, Orange::Rhi::DescriptorType::UniformBuffer,
                                     1, Orange::Rhi::ShaderStage::Fragment});
            lay.mpDebugName = "orange_engine.lens.layout";
            impl.lensLayout = rhi.CreateDescriptorSetLayout(lay);

            Orange::Rhi::BufferDesc ub{};
            ub.mSize        = sizeof(Pipeline::Impl::LensUboData);
            ub.mUsage       = Orange::Rhi::BufferUsage::Uniform;
            ub.mMemoryUsage = Orange::Rhi::MemoryUsage::CpuToGpu;
            impl.lensUbo    = rhi.CreateBuffer(ub);

            if (!impl.lensFs || !impl.lensLayout || !impl.lensUbo)
            {
                ORANGE_LOG_ERROR("Pipeline::Initialize: lens 资源创建失败");
                Shutdown();
                return ResultCode::InternalError;
            }
            ...
            d.mColorBlend.mAttachments.push_back({}); // no blend
            d.mRenderTargets.mColorFormats.push_back(kHdrColorFormat);
            d.mDescriptorSetLayouts.push_back(impl.lensLayout.get());
            d.mpDebugName     = "orange_engine.lens";
            impl.lensPipeline = rhi.CreateGraphicsPipeline(d);
            if (!impl.lensPipeline)
            {
                ORANGE_LOG_ERROR("Pipeline::Initialize: lens pipeline 创建失败");
                Shutdown();
                return ResultCode::InternalError;
            }
        }
        ...
        // 7.6 Main pass descriptor set + light UBO + shadow caster pipeline
        {
            ...
            if (!impl.mainDescLayout || !impl.lightUbo || !impl.pointLightsUbo || !impl.spotLightsUbo || !impl.spotShadowUbo || !impl.mainDescPool)
            {
        ...
        // 7.7 Dummy IBL 资源
        {
            ...
            cubeDesc.mUsage           = Orange::Rhi::TextureUsage::Sampled | Orange::Rhi::TextureUsage::TransferDst;
            ...
            brdfDesc.mUsage       = Orange::Rhi::TextureUsage::Sampled | Orange::Rhi::TextureUsage::TransferDst;
        ...
        // 7.8 set 1 · per-instance material 贴图基础设施（GAP-2026-05-25 A2/G1）
        {
            ...
            auto make1x1 = [&]() -> std::unique_ptr<Orange::Rhi::RHITexture>
            {
                ...
                t.mArrayLayers = 1u;
                t.mUsage       = Orange::Rhi::TextureUsage::Sampled | Orange::Rhi::TextureUsage::TransferDst;
            ...
            if (!impl.materialSampler || !impl.materialTexLayout || !impl.materialTexPool || !impl.defaultWhiteTex || !impl.defaultNormalTex)
            {
