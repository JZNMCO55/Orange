...
glm::vec3 ApplyTonemap(const glm::vec3& hdr, TonemapOperator op) noexcept
{
    switch (op)
    {
        case TonemapOperator::ACES_Narkowicz: return AcesNarkowicz(hdr);
        case TonemapOperator::AgX:            return AgX(hdr);
        case TonemapOperator::Reinhard:       return Reinhard(hdr);
        case TonemapOperator::Linear:         return LinearClamp(hdr);
    }
