...
bool Pipeline::Impl::EnsureMotionBlurResources()
{
    if (renderDevice == nullptr || sceneDepth == nullptr || hdrColor == nullptr
        || hdrSampler == nullptr || motionBlurPipeline == nullptr
        || dofCompositePipeline == nullptr || motionBlurLayout == nullptr
        || bloomLayout == nullptr || motionBlurUbo == nullptr)
    {
        return false;
    }
    ...
    if (!motionBlurPool)
    {
        ...
        if (!motionBlurPool) { return false; }
    }
    ...
    if (motionBlurCompositeSet == nullptr)
    {
        ...
        if (!motionBlurCompositeSet) { return false; }
        motionBlurCompositeSetBound = nullptr;
    }
                                          ...
                                          const glm::mat4& curViewProj)
{
    ...
    // ---- Pass 1：gather（采 hdrColor[ShaderReadOnly] + depth → motionBlurColor）----
    {
        ...
        sc.mWidth = motionBlurColorWidth; sc.mHeight = motionBlurColorHeight;
        cmd.SetScissor(sc);
