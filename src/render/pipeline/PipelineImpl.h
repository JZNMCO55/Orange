...
namespace Orange::Engine::Render
{
    ...
    struct Pipeline::Impl
    {
        ...
        {
            return !mat.textureSlots.empty() || PipelineDetail::ComputePushConstantSize(mat) >= 160u;
        }
        ...
        bool EnsureHdrTarget()
        {
            ...
            t.mUsage    = Orange::Rhi::TextureUsage::RenderTarget | Orange::Rhi::TextureUsage::Sampled | Orange::Rhi::TextureUsage::TransferSrc;
        ...
        bool EnsureViewportTarget()
        {
            ...
            t.mUsage    = Orange::Rhi::TextureUsage::RenderTarget | Orange::Rhi::TextureUsage::Sampled | Orange::Rhi::TextureUsage::TransferSrc; // 允许像素 readback（DebugReadbackPixel）
            auto newTex = renderDevice->GetRhiDevice().CreateTexture(t);
            if (!newTex)
            {
        ...
        {
            if (rttHdr && rttDepth && rttPassthroughSet && rttCachedWidth == w && rttCachedHeight == h)
            {
                return true;
            }
            ...
            if (!rttHdr)
            {
                ORANGE_LOG_ERROR("Pipeline::EnsureRttScratch: CreateTexture(HDR) 失败 ({}x{})", w, h);
                return false;
            }
            ...
            if (!rttDepth)
            {
                ORANGE_LOG_ERROR("Pipeline::EnsureRttScratch: CreateTexture(depth) 失败 ({}x{})", w, h);
                return false;
            }
            ...
            if (!rttPassthroughSet)
            {
                ...
                if (!rttPassthroughSet)
                {
                    ORANGE_LOG_ERROR("Pipeline::EnsureRttScratch: AllocateDescriptorSet 失败");
                    return false;
                }
            }
