...
namespace Orange::Engine::Render
{
    namespace
    {
            ...
            const std::filesystem::path& jsonPath)
        {
            ...
            std::string vertSpv;
            std::string fragSpv;
            if (!reader.ReadString("vertexSpv", vertSpv) || vertSpv.empty() || !reader.ReadString("fragmentSpv", fragSpv) || fragSpv.empty())
            {
            ...
            for (std::size_t i = 0; i < uniformCount; ++i)
            {
                ...
                std::string       uName;
                std::string       uType;
                if (!reader.ReadString(base + "/name", uName) || !reader.ReadString(base + "/type", uType))
                {
            ...
            for (std::size_t i = 0; i < slotCount; ++i)
            {
                ...
                std::string       slotName;
                if (!reader.ReadInt(base + "/binding", binding) || !reader.ReadString(base + "/name", slotName))
                {
    ...
    Result<void, ResultCode> MaterialSystem::RegisterBuiltins()
    {
        ...
        if (!texturedShaderOk || !toonShaderOk || !rimShaderOk || !dissolveShaderOk || !emissiveShaderOk || !pbrShaderOk)
        {
            return ResultCode::IoError;
        }
        return {};
