...
namespace Orange::Engine::Render
{
                                      ...
                                      float outRGBA[4]) const
    {
        ...
        auto& impl = *mpImpl;
        if (!impl.offscreenMode || !impl.viewportColor || impl.renderDevice == nullptr || !impl.offscreenCmd || x >= impl.viewportWidth || y >= impl.viewportHeight)
        {
            return false;
        }

        auto&               rhi    = impl.renderDevice->GetRhiDevice();
        const std::uint64_t pixels = static_cast<std::uint64_t>(impl.viewportWidth) * static_cast<std::uint64_t>(impl.viewportHeight);
                                        ...
                                        std::uint32_t& outW, std::uint32_t& outH) const
    {
        ...
        auto& impl = *mpImpl;
        if (!impl.offscreenMode || !impl.viewportColor || impl.renderDevice == nullptr || !impl.offscreenCmd || impl.viewportWidth == 0 || impl.viewportHeight == 0)
        {
            return false;
        }
        ...
        const std::uint64_t bytes = static_cast<std::uint64_t>(w) * static_cast<std::uint64_t>(h) * 4u; // BGRA8
                                                       ...
                                                       std::uint32_t              height)
    {
        ...
        struct RttScope
        {
                     ...
                     std::uint32_t w, std::uint32_t h) : impl(i)
            {
                ...
                savedHdrW                           = impl.hdrWidth;
                savedHdrH                           = impl.hdrHeight;
                savedPendingW                       = impl.pendingWidth;
                savedPendingH                       = impl.pendingHeight;
            ...
            ~RttScope()
            {
                ...
                impl.pendingWidth                   = savedPendingW;
                impl.pendingHeight                  = savedPendingH;
                impl.hdrWidth                       = savedHdrW;
                impl.hdrHeight                      = savedHdrH;
                std::swap(impl.passthroughSet, impl.rttPassthroughSet);
                std::swap(impl.sceneDepth, impl.rttDepth);
                std::swap(impl.hdrColor, impl.rttHdr);
            }
    ...
    void Pipeline::Shutdown()
    {
        ...
        // TAA 资源（set 先于 pool）。
        for (auto& s : impl.taaResolveSet)
        {
            s.reset();
        }
        for (auto& s : impl.taaCopySet)
        {
            s.reset();
        }
        impl.taaPool.reset();
        for (auto& h : impl.taaHistory)
        {
            h.reset();
        }
        ...
        // error → SEGFAULT）。
        for (auto& v : impl.shadowMapLayerViews)
        {
            v.reset();
        }
        ...
        for (auto& v : impl.spotShadowLayerViews)
        {
            v.reset();
        }
        ...
        for (auto& v : impl.pointShadowFaceViews)
        {
            v.reset();
        }
        for (auto& c : impl.pointShadowCubes)
        {
            c.reset();
        }
    ...
    void Pipeline::SetVfxSystem(VfxSystem* system) noexcept
    {
        ...
        if (system != nullptr && !system->IsInitialized() && mpImpl->renderDevice != nullptr && mpImpl->assets != nullptr)
        {
                                    ...
                                    ::Orange::Engine::Asset::AssetRegistry& assets)
    {
        ...
        if (pixelBytes != static_cast<std::size_t>(equirectW) * static_cast<std::size_t>(equirectH) * ::Orange::Engine::Asset::BytesPerPixel(::Orange::Engine::Asset::TextureFormat::R32G32B32A32_Float))
        {
            ORANGE_LOG_WARN("Pipeline::BakeIblFromWorld: equirect byte size {} 与 {}x{}x16 不符，"
                            "回退到 dummy IBL",
                            pixelBytes, equirectW, equirectH);
            SetIblTextures(nullptr, nullptr, nullptr);
            return;
        }
        ...
        equirectDesc.mUsage       = Orange::Rhi::TextureUsage::Sampled | Orange::Rhi::TextureUsage::TransferDst;
        ...
        for (const auto& drawable : impl.scene.Drawables())
        {
            ...
            if (meshAsset != nullptr && meshAsset->HasSubMeshes())
            {
                ...
                for (const auto& sm : meshAsset->SubMeshes())
                {
                    ...
                    if (sm.materialSlot < drawable.subMeshMaterials.size() && drawable.subMeshMaterials[sm.materialSlot] != nullptr)
                    {
    ...
    bool Pipeline::Impl::RecordPassthroughToViewport()
    {
        ...
        if (bloomPass != nullptr && impl.bloomMipsReady && impl.bloomCombineSet && impl.tonemapPipeline)
        {
    ...
    void Pipeline::Impl::RenderOffscreen(Orange::Engine::World& world)
    {
        ...
        if (impl.scene.HasCamera())
        {
            ...
            else
            {
                ...
                for (auto& m : impl.cascadeViewProjs)
                {
                    m = glm::mat4(1.0f);
                }
        ...
        if (impl.scene.HasCamera())
        {
            ...
            const glm::mat4 viewProj    = impl.ApplyTaaJitter(impl.scene.MainCamera().projection) * impl.scene.MainCamera().view;
            ...
            const glm::mat4 baseViewProj    = impl.scene.MainCamera().projection * impl.scene.MainCamera().view;
            ...
            // 丢弃），故必须紧贴主 pass 之前。
            if (ok && (impl.FindActiveSsaoPass() != nullptr || impl.FindActiveSsrPass() != nullptr || impl.FindActiveContactShadowPass() != nullptr))
            {
            ...
            if (ok && impl.vfxSystem != nullptr && impl.vfxSystem->IsInitialized() && impl.hdrColor)
            {
            ...
            if (ok && impl.pAuxPassProvider != nullptr && impl.offscreenCmd != nullptr && impl.hdrColor != nullptr && impl.sceneDepth != nullptr && impl.hdrSampler != nullptr)
            {
    ...
    void Pipeline::Impl::EnsureMeshGpuCache()
    {
        ...
        for (const auto& drawable : impl.scene.Drawables())
        {
            ...
            vbDesc.mUsage       = Orange::Rhi::BufferUsage::Vertex | Orange::Rhi::BufferUsage::Transfer;
            ...
            ibDesc.mUsage       = Orange::Rhi::BufferUsage::Index | Orange::Rhi::BufferUsage::Transfer;
    ...
    void Pipeline::Impl::SyncPostProcessFromWorld(Orange::Engine::World& world)
    {
        ...
        for (const auto& h : localHits)
        {
            ...
            if (w >= 0.5f)
            {
                result.motionBlurSampleCount = pp.motionBlurSampleCount;
            }
        ...
        if (hdrReady && impl.scene.HasCamera())
        {
            ...
            else
            {
                ...
                const glm::mat4 viewProj =
                    impl.ApplyTaaJitter(impl.scene.MainCamera().projection) * impl.scene.MainCamera().view;
                ...
                if (offscreenOk && impl.scene.HasCamera() && (activeSsao != nullptr || activeSsr != nullptr || impl.FindActiveContactShadowPass() != nullptr))
                {
                ...
                if (offscreenOk && impl.vfxSystem != nullptr && impl.vfxSystem->IsInitialized() && impl.hdrColor)
                {
                    impl.vfxSystem->DrawParticles(
                        impl.offscreenCmd.get(),
                        impl.hdrColor->GetDefaultView(),
                ...
                if (offscreenOk && impl.pAuxPassProvider != nullptr && impl.offscreenCmd != nullptr && impl.hdrColor != nullptr && impl.sceneDepth != nullptr && impl.hdrSampler != nullptr)
                {
                    if (!impl.sceneDepthLayoutShaderReadOnly)
                    {
                        cmd.TransitionTexture(*impl.sceneDepth,
