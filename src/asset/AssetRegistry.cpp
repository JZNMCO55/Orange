...
namespace Orange::Engine::Asset
{
    ...
    void AssetRegistry::Impl::WorkerLoop()
    {
        for (;;)
        {
            ...
            jobsCv.wait(lock, [this]
                        { return shutdown.load() || !jobs.empty(); });
            ...
            if (idx >= tab.slots.size() || !tab.slots[idx].live || tab.slots[idx].path != job.path)
            {
    ...
    {
        if (loaderRaw == nullptr || loaderDeleter == nullptr || invoker == nullptr || assetDeleter == nullptr)
        {
                                         ...
                                         std::uint64_t&        outHandle)
    {
        ...
        if (!slotAfter.live || slotAfter.path != pathKey || slotAfter.status != LoadStatus::Pending)
        {
            if (assetRaw && loaderSnapshot.assetDeleter)
            {
                loaderSnapshot.assetDeleter(assetRaw);
                                      ...
                                      std::int64_t          timeoutMs) const noexcept
    {
        ...
        auto pred = [&]()
        {
            return table.slots[idx].status != LoadStatus::Pending || mpImpl->shutdown.load();
        };
