...
- **状态**：渲染管线接通 + OE 端 follow-up 缺口待补（2026-05-19）—— B.1 全 ✅，B.2 代码全部落地，sample segfault 真因（OR 端 cache lifecycle dangling raw pointer）已解。起草于 2026-05-17，6 项 review 议题草稿期全部决议（见末尾决议记录）。**B.1 ✅（2026-05-18）**：Task PBR-01 / 02 / 03 落地（commits 753627e / edc783b / 0bc55c5 / 080ed8f / 97d4950 / 4fcabe0）；OrangeRender R1/R2/R3/R4 audit 同期 ✅（vendor bump 06025ae）。**B.2 代码 ✅（2026-05-18）**：Task PBR-04 / 05 / 06 落地（commits f9c0e84 / eae6479 / f063829 / 89bee10 / bfb0087 / 6c823a2 / 736c253）+ Task PBR-07 sample 14_pbr_ibl + `Pipeline::BakeIblFromWorld` 高阶 API 代码落地（cf75126 / b92d2d9）；c10-c12 三轮调查锁定 segfault 真因 = OR `mPipelineCache` 持有 desc raw RHI* 指针 + caller-owned 对象析构后 dangling deref（详 OR `vendor/OrangeRender/docs/incoming_bugs.md` `BUG-2026-05-18-pipeline-cache-dangling-desc-raw-pointer` 处理记录段）。**OR fix 落地 + 本仓消费（2026-05-19）**：OR commit `5715a9d` (T1+T2 cache key resolved + value keepalive) + `4eec1bf` 归档；本仓 vendor submodule bump 到 `4eec1bf` + rebuild OE 后 sample 完整跑通 init + frame loop，但视觉验收暴露 2 个新 OE 端 follow-up 缺口阻塞 Task 06.5-07 ✅：`docs/engine-known-gaps.md` GAP-2026-05-19-pbr-ibl-specular-quality（IBL specular 高 roughness 能量损失 + 中 roughness 采样伪影；direct GGX 路径无此症状）+ GAP-2026-05-19-editor-environment-component-wiring（Editor Inspector 字段 schema ✅ 但 AssetRef drop/picker 无实现 + Intensity wiring 不响应）。**Task 06.5-07 仍不 ✅ 直到 2 GAP 落地后视觉验收全过**；B.1 / B.2 acceptance-checklist 落 `docs/acceptance/phase-6.5-B.{1,2}-acceptance-checklist.md`（B.2 末段已更新 OR fix 后的 6 项实测）。v0.8 编辑器伴随 milestone 立项继续推到 Task 06.5-07 ✅ 后
