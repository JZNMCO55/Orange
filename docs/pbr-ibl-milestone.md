...
- **状态**：接近完工（2026-05-18）—— B.1 全 ✅，B.2 代码全部落地但 sample 视觉验收抓到 segfault 待跨仓修。起草于 2026-05-17，6 项 review 议题草稿期全部决议（见末尾决议记录）。**B.1 ✅（2026-05-18）**：Task PBR-01 / 02 / 03 落地（commits 753627e / edc783b / 0bc55c5 / 080ed8f / 97d4950 / 4fcabe0）；OrangeRender R1/R2/R3/R4 audit 同期 ✅（vendor bump 06025ae）。**B.2 代码 ✅（2026-05-18）**：Task PBR-04 / 05 / 06 落地（commits f9c0e84 / eae6479 / f063829 / 89bee10 / bfb0087 / 6c823a2 / 736c253）+ Task PBR-07 sample 14_pbr_ibl + `Pipeline::BakeIblFromWorld` 高阶 API 代码落地（cf75126 / b92d2d9）；但 `14_pbr_ibl.exe --furnace` 第一次 Render 采样 baked IBL 时 segfault（exit 139），二分定位排除本仓内可能性后登记 OrangeRender `vendor/OrangeRender/docs/incoming_bugs.md` BUG-2026-05-18-baked-ibl-cube-sampling-segfault；按 CLAUDE.md 单 session 双向操作禁令，修复留另开 session 在 OrangeRender 仓 deep debug。**Task 06.5-07 不 ✅ 直到 sample runs**；B.1 / B.2 acceptance-checklist 落 `docs/acceptance/phase-6.5-B.{1,2}-acceptance-checklist.md`（B.2 末段记已知 bug）。v0.8 编辑器伴随 milestone 立项推到 sample crash 修复 + 06.5-07 ✅ 后
