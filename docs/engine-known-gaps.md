...
- **剩余**：G3（glTF material 自动导入，Inc5）代码已成、端到端待真实 glTF 资产验；acceptance checklist + CMake VERSION bump 待 Orange-Wiki 子仓单独 session（单子仓纪律）。
  - **2026-05-30 ✅ G3 端到端验证 + AO bug 修复**（OE `5c2c06d`，"干到十点" goal session）：把 "glTF material → `.material`" 导入链锁进 headless 端到端测试，并修一个**确定的 bug**。
    ...
    - **顺带确认的其他 glTF 导入 gap**：① 多 material per mesh —— `RunGltfImport` 合并 primitive 成单 MeshAsset 只取 firstMat，多材质模型丢非首材质（需引擎 sub-mesh + 多 material slot）【留后续】；② tangent fallback（缺 UV/normal 落 Lengyel）无端到端验证【留后续】；③ `.glb` 内嵌贴图 / data: URI 贴图不支持 —— **✅ 2026-05-31 落地（OE `bf10ea5`）**，见下方"内嵌贴图落地记录"。
  - **✅ 2026-05-31 `.glb` 内嵌贴图 / data: URI 导入落地**（OE `bf10ea5`，goal session "自主打磨引擎 gap"；中断 agent 起草 + 主循环复核修两个真 bug + 验证收口）：此前 importer 只认外部 uri 贴图，`.glb` 内嵌（buffer_view）/ data: URI 的 `image->uri` 为空 → resolver 返空 → 贴图槽落 default 白贴图。**Blender 默认导出 `.glb` 且内嵌贴图，正是"场景从 Blender 做起"工作流的要害。**
    - **解析层**（`GltfMaterialParse`，headless seam）：加 `ResolveImageIndex`（判内嵌 buffer_view/data: → `images[]` 下标）+ `GltfMatInfo.*ImageIndex` 字段；`ExtractGltfMaterial` 加 `data` 参数（默认 nullptr，旧 caller 兼容）。seam 仍只解析"贴图引用标识"不碰文件——内嵌字节提取留 importer 层。
    - **提取层**（`GltfImporter`）：加 `DecodeBase64` / `ReadEmbeddedImageBytes`（GLB buffer_view 优先、其次 data: URI）/ `EmbeddedImageExtension`（mime/magic 判 png/jpg）/ `ExtractEmbeddedImageToDisk`。**必须在 `cgltf_free` 之前**取字节写到模型自己的 `assets/Models/<stem>/`，回填 `matInfo.*Src`，之后与外部贴图走同一条 `ImportTextureToRegistry` co-locate 流程。同一 image 多槽引用按 `images[]` 下标缓存只写盘一次。
    - **顺带修一个通用 bug**：`ImportTextureToRegistry` 加 same-file 守卫（`fs::equivalent`）——内嵌贴图已落在 modelDir、再 co-locate 到同目录即 `src==dest`，旧代码 `copy_file(self,self)` 报错 → 贴图槽又落空（内嵌没真修上）。守卫后跳过 copy 直接 Load+`.meta`；外部贴图首次导入 `dest` 不存在仍照常 copy，零回归。
...
- **G3 · PointLight 全向 cubemap 阴影** ✅：N 个独立 6-layer `TexCube`（cap 2，binding 9/10 `samplerCube`）；per-face 90° perspective depth-only；`pbr.frag` 按方向采 + dominant 轴距离重建 NDC depth 比较。用独立 cube 而非 cubeArray 免 `imageCubeArray` feature（见下「跨仓核对」+ 已登记 OrangeRender FEATURE）
  ...
  - **后续 upgrade（cube-array，device feature 已就绪，2026-05-30 评估为 P3 暂不实施）**：现状 = N 个独立 `samplerCube`（`pbr.frag.glsl:108-109` 硬编码 `uPointShadowCube0/1` + `SamplePointShadow:184-186` 静态分支 `cubeIndex==0?cube0:cube1`）。**关键认知**：`kMaxPointShadowCasters=2` 这个上限**不是显存约束，而是被 shader 写死的两个 sampler 变量 + 静态分支锁死**——GLSL 不能动态索引 sampler 变量，加第 3 个 caster 必须再写一个 `uPointShadowCube2` + 一层分支。FEATURE-2026-05-26-enable-image-cube-array 已落地（OrangeRender `7bc8c57` 启用 `imageCubeArray`，umbrella pointer 已 bump，SDK 已 reinstall 到 `D:/3rdparty/install`，debug-view wireframe session 已实证此 SDK 可消费 device feature），前置条件齐备。**切 `samplerCubeArray` 的精确改动清单（4 处，可直接照做）**：
    1. `pbr.frag.glsl` —— binding 9/10 两个 `samplerCube` → 单 binding 9 `samplerCubeArray uPointShadowCubes`；`SamplePointShadow`（173）去静态分支改 `texture(uPointShadowCubes, vec4(L, float(cubeIndex))).r`；注释 102-106 同步。
    2. `PipelineShadow.cpp::EnsurePointShadowCube`（773-845）—— N 个独立 `TexCube`(6 layer) → 单 `TexCube`(`mArrayLayers = 6*kMaxPointShadowCasters = 12`；RHI 对 arrayLayers>6 的 `TexCube` 已自动生成 CUBE_ARRAY 默认 view，见 OrangeRender `CreateVulkanTexture`)；per-face depth view `mBaseArrayLayer` 从 `face` → `c*6+face`；descriptor write 从 N 次（各绑 `kPointShadowBinding0+c`）→ 1 次（整个 array 默认 view 绑 binding 9）。
    3. `PipelineShadow.cpp::RecordPointShadowPass`（847-966）—— per-(caster,face) RT 仍用 `pointShadowFaceViews[c*6+face]`（flat index 已对，逻辑不变）；唯 `TransitionTexture` 目标从 N 个独立 texture → 单 array texture（layout 转换改整体或 per-layer-range）。
    4. `PipelineSetup.cpp` descriptor layout（1321-1328）—— 循环 push N 个 binding → 单 binding；pool size（1370）`5 + kMaxPointShadowCasters` → `5 + 1`。
    - **收益**：解除"扩展 point shadow caster 数 ⇒ 改 shader（加 sampler 变量 + 加静态分支）"的耦合——切 array 后提 `kMaxPointShadowCasters` 只需改常量 + array layer 数，shader 零改动（O(N) 代码改动 → O(1) 常量）。
    - **代价/风险**：动的是已 dogfood 验证过的全向阴影核心（参上一条「后续修复」那次 -Y 面真 bug），改完**必须重新 dogfood**（`samples/16_light_family_shadows` point 全向 radial 阴影 + `ShadowOcclusionTest` 多 face 主轴回归门）。
    - **优先级 P3（低）**：当前 cap=2 够用，**单独切 array 无即时功能收益**（cap 不变），仅架构优雅。**待"首游 / 编辑器实际需要 >2 个投影点光"拉动时再做**，届时连同 cap 提升一起改、dogfood 一次到位，避免为消费 device feature 而平白增加一个阴影 dogfood 项。device feature 端到端闭环已由 debug-view wireframe（`fillModeNonSolid`）达成，imageCubeArray 这半不必强行单独闭环。
...
- **G3（point cubemap shadow）**：**纯 Engine** ✅（与初步评估的"跨仓风险点"不同）。RHI 已支持 **render-to-cube-face depth**：`TexCube` + DepthStencil usage + per-face Tex2D depth view（IBL prefilter 用同款 render-to-cube-face color 机制；depth 走相同 view 路径，aspect 自动）。**唯一缺口**：`samplerCubeArray` 需 `imageCubeArray` device feature（OrangeRender 未启用）→ 用 N 独立 `samplerCube` 绕过（IBL cube 已在用，core 能力）。已登记 `OrangeRender/docs/incoming_feature.md` FEATURE-2026-05-26-enable-image-cube-array（低优先 nice-to-have，启用后可合并为 cubeArray + 扩展点光阴影数）。**2026-05-30 update**：该 FEATURE 已落地（OrangeRender `7bc8c57` 启用 `imageCubeArray`，umbrella 已 bump + SDK 已 reinstall）；切 cubeArray 的精确 4 处改动清单 + 收益/风险已登记于上方 G3「后续 upgrade」子条，评估 P3 暂不实施（待 caster 上限真要 >2 时连同 cap 提升一起做）
...
**scene snapshot 缩略图 ✅ 已落地**（2026-05-30，OE `4450e36`）：Asset Browser `.scene.json` 从 `[S]` 文本变真实场景预览。`ThumbKind::Scene` —— `BakeSceneThumbnail`（ReadFileToString 读 `.scene.json` → LoadFromString 到常驻 scene-scratch → AABB 框相机 → RenderToTexture）；**多根清理 SceneGuard**（scene 多根，析构遍历 created 逐个 DestroySubtree，IsValid 跳已连带销毁的，防累积污染——测试断言 residentAfter=2 无残留）；scene 自带 camera 忽略用 scratch 框定 + 自带 light 生效。**至此 ThumbnailService 四类（material/prefab/mesh/scene）全闭环**。`SceneThumbnailBakeTest`，ctest 71/71 四类零回归。

**剩余（后续迭代 / session）**：override（逐属性 diff vs 整体解耦，链接组件已留 instanceId 锚）+ 蓝条 UI + 右键 apply-revert / 嵌套 prefab / 缩略图磁盘缓存（跨 session 复用免重烘）/ 挂任意 parent 实例化（用户现可实例化后自己 `ReparentTo`）。
...
- **状态**：✅ **已落地 2026-07-01**（headless 真验，非 dogfood-pending）。`PhysicsWorld` 新增 5 个查询方法，全部走 Box2D 3.x 原生函数、公共面止步 `BodyHandle`（不暴露 box2d / entt 类型，沿 header isolation）：
  ...
  - **shape-cast 扫掠查询 ✅ 2026-07-01**（follow-up commit）：`ShapeCastCircle(origin, radius, dir, maxDist)` / `ShapeCastCapsule(p1, p2, radius, dir, maxDist)` → `RaycastHit`（最近命中）。底层 `b2World_CastShape` + `b2MakeProxy`（圆=单点、胶囊=两端点 + 半径），复用 RaycastHit + EncodeBodyHandle。比 raycast 更抗穿透（扫掠能逮到比自身细的缝隙旁碰撞体），服务角色移动 / 贴地检测。忽略 initial-overlap 零法线（同 RaycastClosest）；退化输入不崩。`box2d.h` 已传递 include `collision.h`，无新 box2d include。physics_query_test +5 子测试（含"扫掠优于射线"判别用例）。
  - **sensor / contact 事件 ✅ 2026-07-01**（follow-up）：`GetSensorBeginEvents`/`GetSensorEndEvents`→`SensorEvent{sensor,visitor}`、`GetContactBeginEvents`→`ContactBeginEvent{bodyA,bodyB,point,normal}`、`GetContactEndEvents`→`ContactEndEvent{bodyA,bodyB}`。走 `b2World_GetSensorEvents`/`GetContactEvents`；`MakeShapeDef` 开 `enableSensorEvents`+`enableContactEvents`（此前默认 false 故不触发）。end 事件 shape 可能已销毁 → `ShapeToBodyHandle` 先 `b2Shape_IsValid`。触发器/拾取/落地检测的核心玩法原语。
