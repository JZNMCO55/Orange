...
- **状态**：✅ **已落地 2026-07-01**（headless 真验，非 dogfood-pending）。`PhysicsWorld` 新增 5 个查询方法，全部走 Box2D 3.x 原生函数、公共面止步 `BodyHandle`（不暴露 box2d / entt 类型，沿 header isolation）：
  ...
  - **剩（可选后续）**：collider category/mask 位 → 查询暴露 `QueryFilter`（当前所有 body 默认类，暴露无意义）；shape 粒度句柄（当前 1 body = 1 shape，返回 body 粒度足够）。消费侧：OrangeGames 首游用它替换 `LevelBox` analytic workaround 属游戏仓工作，另仓 session 做。
