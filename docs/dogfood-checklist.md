...
- **范围限制（dogfood 时注意，不是 bug）**：
  - **已切：mesh 渲染 + picking**。**未切：gizmo 放置/拖动、光源方向、物理 collider**（后续 increment）——具体表现：
    ...
    - **DirectionalLight 方向已随父传播 ✅**：把平行光 parent 到旋转父下、转父 → 光方向跟着转（阴影/明暗方向变）。root 灯零回归。**point/spot 灯位置 + spot 方向、collider 仍未切**——parent 到非原点/旋转父下不跟随（mesh+directional 跟随）。glTF 导入的灯（通常 root 子）方向仍对；非 root 导入的 directional 灯方向可能偏（rare，待 importer R-bridging）。
