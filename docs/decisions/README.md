...
| ADR-020 | OrangeEditor MCP 实时协同桥架构 —— 进程内 TCP 命令端 + 独立 Python MCP server + 主线程帧末队列 + 命令栈/EntityGuid/schema 复用；首版读写协同闭环（用户拍板做 + 范围，关键选型待确认） | proposed | 2026-06-12 | [Wiki](../../../Orange-Wiki/case-studies/orange-engine/decisions/ADR-020-editor-mcp-realtime-coediting-bridge.md) |
