...
- **里程碑**：
  ...
  - **A1.1 引擎核心**（分两步降风险）：
    ...
    - **step 2 进行中**：drawable / picking / light / physics / gizmo **逐个**改读 `WorldTransformComponent`：
      ...
      - **装饰 gizmo overlay ✅ 2026-06-02**：DirectionalLight 箭头 / PointLight 圆环 / SpotLight 锥体 / PostProcess volume box / ParticleEmitter spawn box 的 origin+方向读 world（CameraFrustum overlay 涉朝向、niche，留 follow-up）。
      - **gizmo（变换） / physics 待切**（剩余两项，都需 world→local on write/apply 方向，较难）：transform gizmo（translate/rotate/scale + GizmoContext + group + snap + space）画在 local 偏移处 + 拖动写 local（parented 非原点父需 world 放置 + world→local apply，可复用 EditorHierarchy ComputeWorldMatrix）——**GUI 无法 headless 验证 + 是编辑器主操作工具，盲改风险高，未做**；物理 collider（2D + 双向 sim 写回需 world→local，subtler）。非 root 导入的 directional/spot 灯需 importer R-bridging（angleAxis(90°,X)）才完全对（rare）。dogfood item 31「已知未切」。
      - **小结**：**所有「读」类位置/方向 consumer + 装饰 overlay 已切**（render mesh / picking / 3 类光源 + halo + postprocess volume + 5 个 gizmo overlay），root 实体全零回归。**只剩 transform gizmo + physics 两个「写/apply」类**（world→local 方向，intricate + GUI/双向无法 headless 验证，盲改风险高）。A1 ~90% 落地。
