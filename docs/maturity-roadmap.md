...
- **里程碑**（XL，多 session）：
  ...
  - **B2.1 动画数据模型 ✅ 已落地（数据模型 + animator 接入 + 编辑器数据原语）2026-06-02**：`AnimationClip`/`AnimationTrack`/`Keyframe` + `InterpMode`{Step/Linear/Bezier} + header-only `SampleTrack`（二分 + 插值 + clamp，`include/orange/engine/animation/AnimationClip.h`）；`ProceduralAnimator::AddDataChannel`（接现有 channel，runtime 零改）；**编辑器数据层原语齐备**：Sort/IsSorted（升序不变量）+ ComputeClipDuration/WrapClipTime（playback loop/clamp）+ AddKeyframeSorted（打键 C/U）+ Find/RemoveKeyframe（选/删）+ MoveKeyframeTime（dopesheet 拖）。headless 全测（AnimationClipTest 13 例 + ProceduralAnimator 数据 channel 例），ctest 77/77。**剩**：写 TransformComponent 的 `ClipAnimator`（靠 A1 hierarchy 传播子跟随）+ `.anim` 序列化（走 Core::Serialization）+ timeline/曲线 UI（dogfood）。
