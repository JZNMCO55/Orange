...
- **M2 · 写闭环** ✅（2026-06-12 落地）：`create_entity`（CreateEntityCommand）/ `set_field`（schema get/set → SetFieldValueCommand\<T\>，11 种可写 PropertyType + range clamp + Play 期写保护 allowInPlay）/ `add_component`（schema.add，**镜像编辑器现实=清栈不可 undo**）/ `delete_entity`（pendingDelete，不可 undo）/ `select_entity`（UI 状态）/ `save_scene`（pendingSceneOp=Save）。端到端真验（smoke_test 写往返）：create→set_field(position=[1,2,3])→get_entity 确认写入生效→错误组件 graceful→add_component(Renderable) 挂上→select→delete 后实体确实从场景消失；save_scene 带显式路径真写盘 30KB。**首版（P0 共 11 tool，读写协同闭环）到此完整**——AI 能连接 / 看场景结构 / 看 viewport 画面 / 改实体字段 / 截图自验。set_field 走命令栈（用户可 Ctrl+Z 撤 AI 操作）。ctest 95/95 零回归，lint 绿。剩真实 Claude 客户端 dogfood（checklist item 59，尤其 Ctrl+Z 撤 AI 操作的 GUI 验证）。
- **M4 · P1 全 17 tool** ✅（2026-06-12 落地）：覆盖需求 §4.M 的 P1 集，分三组——
  - **E1 协同效率包**：`get_editor_state`（playState/scenePath/dirty/selectedGuid/gizmo 模式）/ `find_entities`（name 子串 + 组件 + underGuid 子树过滤）/ `get_camera`·`set_camera`（轨道相机读写 + 越界 clamp + Standard-Views 构图）/ `frame_entity`（复用新增公共 `FrameEntityCamera`，不传 guid=Frame All，不改 selection）/ `duplicate_entity`（复用 SaveSubtreeToString+SeparateClonedIdentities，可 Undo，返回克隆根 guid）/ `reparent_entity`（复用 EditorHierarchy keep-world 变体，可 Undo，IsAncestorOf 防环）/ `remove_component`（复用导出的 `SchemaInspector::CaptureComponentValues`，有 add+get 的可 Undo）/ `begin_undo_group`·`end_undo_group`（CommandStack BeginGroup/EndGroup + **30s 超时 / 客户端断连自动闭合护栏** `TickMcpUndoGroupGuard`）。
  - **E2 资产管线**：`list_assets`（递归扫 assets/ 按 AssetKind+pathPrefix 过滤，返回可直接喂 AssetRef 的相对路径）/ `import_asset`（复用 ImportDispatcher，新增 `Dispatch(host, scale)` 重载透传 FBX importScale，同步执行直接拿产物路径）。
