...
- Task 10-03 · 屏幕空间环境光遮蔽（SSAO，强化室内场景）✅（2026-05-26 落地：`SsaoPass` 挂进 PostProcessChain；从 sceneDepth 重建 view-space pos + 采法线预通道（10-08）的真实几何法线 + 半球 kernel + 4×4 模糊 + 乘法 blend；window + 编辑器 offscreen 两路径均生效。已知简化 = AO 作用于含直接光的 HDR 而非仅 ambient，严格 ambient-only 需 G-buffer 拆 albedo/ambient。见 `samples/16_light_family_shadows --no-ssao` 对比。**2026-05-26 追加 GTAO（Ground Truth AO，Jimenez 2016）**：`SsaoPass.useGtao` 切换，horizon-based + 余弦加权弧积分，比半球 kernel 更准更平滑；复用同一 ssaoLayout/ssaoSet/apply 路径，仅 pass-1 pipeline 换 gtao.frag。坑点 = 投影 y-flip 导致 view-space slice 方向需 omega.y 取负，否则切线基线错位全局过暗（已修）。见 `--no-gtao` 对比）
- Task 10-04 · GPU 粒子（迁移 Phase 5 的 CPU 粒子到 compute shader）
- Task 10-05 · 高质量软阴影（PCSS / VSM）✅（2026-05-26 PCSS 落地：`shadow_pcf.glsl.inc` 加 `SamplePcssShadow` / `SamplePcssShadowArray`——blocker search → penumbra 估计（受影体离遮挡面越远半影越宽，接触处硬）→ 可变半径 PCF（固定 5×5 search + 5×5 PCF，偏移按半径缩放，无动态 loop）。`ShadowConfig.pcssLightSize`（texel 单位，0=关→退回固定 PCF，默认行为不变）经 `uShadowParams.z` 喂入。directional + spot 消费；point cubemap 仍走固定 PCF（cube PCSS blocker search 跨面过繁，留后）。已知简化 = penumbra 用 NDC 深度比例近似（ortho 精确 / perspective 定性正确）。见 `samples/16_light_family_shadows --no-pcss` 对比。VSM 未做）
...
- Task 10-13 · bloom 接编辑器 offscreen 视口（WYSIWYG 收口）✅（2026-05-26 落地：编辑器 offscreen 路径（RenderOffscreen）此前 main→passthrough(ACES) **跳过 bloom**，使编辑器视口与窗口最终画面 bloom 观感不一致。现 RenderOffscreen 在 DoF 之后、god rays 之前接 `EnsureBloomResources` + `RecordBloomChain`（与 window 路径同位）；`RecordPassthroughToViewport` 在活动 BloomPass + bloomMipsReady 时改走 `tonemapPipeline` + `bloomCombineSet`（HDR + bloom → ACES，push exposure=1 + bloomIntensity），否则纯 passthrough(ACES)。**零新 pipeline**——复用窗口 tonemapPipeline（viewportColor 与 swap-chain 同 BGRA8Unorm，格式合法）。至此编辑器视口 post 栈与窗口对齐（SSAO/GTAO·SSR·接触阴影·DoF·TAA·色彩分级·bloom 全接）。验证：ShadowOcclusionTest 离屏 post smoke 加 BloomPass 后中心非黑 + 无 validation 错误。已知简化 = bloom mip 用 pre-TAA/grade 的 HDR（与 window 同款 staleness，demo 无强 emissive 可忽略））
- Task 10-12 · 色彩分级（color grading）✅（2026-05-26 落地：`ColorGradePass` 挂进 PostProcessChain；线性 HDR、tonemap 之前两段——grade（hdrColor + ubo → gradeColor：曝光 stops → 白平衡 RGB 乘子（host 由 temperature/tint 算）→ 对比度（围绕线性中灰 0.18）→ 饱和度（向 luma 收/放））+ composite（gradeColor replace 回 hdrColor，复用 dofCompositePipeline）。放在所有 post（含 TAA）之后、tonemap 之前作最终 look 调整。已知简化 = 线性空间 grade（display-space LUT 留后）。window + offscreen 两路径。见 `samples/16_light_family_shadows --no-grade` 对比）
- Task 10-11 · TAA（时序抗锯齿）✅（2026-05-26 落地：`TaaPass` 挂进 PostProcessChain；每帧投影叠加 Halton(2,3) sub-pixel jitter（法线预通道 + 主 pass 用 jittered viewProj），resolve 把当前帧与重投影的上一帧历史按 feedback(0.9) 混合 + 3×3 邻域 clamp → 写 curHistory，再 copy 回 hdrColor（复用 dofCompositePipeline）。taaHistory[2] ping-pong（写[p] 读[1-p]）。多帧累积 = 超采样 → 抗锯齿 + 把 GTAO/接触阴影/景深的 jitter 噪点去噪。坑：① jitter 注入控制在 viewProj 一处（最小侵入）；② prevHistory 首次使用前 Undefined→ShaderReadOnly（descriptor 引用即需合法 layout，即便 hasHistory=0 不采样，曾触发 validation error）；③ sample capture warmup 提到 32 帧让时序收敛。已知简化 = 仅相机重投影（无 per-object 运动矢量，运动物体靠 clamp 兜底）。window + offscreen 两路径。见 `--no-taa` 对比）
- Task 10-10 · 景深（depth of field）✅（2026-05-26 落地：`DofPass` 挂进 PostProcessChain；两段——gather（hdrColor + depth + ubo → dofColor，从 sceneDepth 算 CoC = saturate(|viewDepth-focusDistance|/focusRange)，按 CoC 半径 16-tap 黄金角螺旋圆盘 gather 模糊）+ composite（dofColor replace 写回 HDR）。独立 dofColor 避免 gather 读写同 target 反馈。对焦面锐利、离焦渐糊。已知简化 = 单层 gather（不分 near/far）+ 圆盘均匀权重（无 bokeh / 前景散射），对焦边界轻微 bleeding。window + offscreen 两路径，god rays / bloom 之前。见 `samples/16_light_family_shadows --no-dof` 对比）
- Task 10-09 · 接触阴影（contact shadows，屏幕空间向光源射线步进硬阴影）✅（2026-05-26 落地：`ContactShadowPass` 挂进 PostProcessChain；单 fullscreen pass 从 sceneDepth 重建 view pos，向 view-space 朝光向 march，命中遮挡输出 <1 因子，乘法 blend 进 HDR；补 shadow map + PCSS 在接触处因 depthBias 抬起留下的漏光缝隙（UE Contact Shadows）。坑：① 起点抖动（hash）打散低步数 banding；② **N·L fade**——采法线预通道（10-08）的法线，背光面（N·L≤0）不叠接触阴影，否则终结线 / 球自遮蔽处重复暗化出大 crescent；③ 法线预通道 gating 需把 contact 算进去（否则 normalBuffer 未渲染 = UNDEFINED）。仅 directional light；window + offscreen 两路径。已知简化 = 乘进整个 HDR 非仅 directional。见 `--no-contact` 对比）
