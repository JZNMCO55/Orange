...
### M6 · workspace / 项目模型（OE，M，ADR-022）—— **核心 ✅ 2026-07-07**（`--project` 任意 cwd 打开端到端验证）
...
- **M6b 分期（后续隔离 session）**：`engine://`/`project://` 虚拟路径 scheme 进 loaders + asset 浏览器 + **scene schema 1.19→1.20 资产路径根相对化 + 向后兼容**（风险最高，触已 ship schema，隔离到 dogfood 在场的专注 session）+ per-project imgui.ini/editor_settings + File→Open Project/Recent Projects 菜单（mid-session 项目切换 = 重 chdir/重配，GUI dogfood-pending）+ OG `slime.orangeproject` 消费 + 删 SlimeEditor POST_BUILD 拷贝 workaround（OG 代码，独立 session）。
...
3. **Game/Scene 双视口**（L，**仍 defer**）：Game 面板（游戏相机）与 Scene 面板（编辑器相机自由飞，Play 期可旁观）并存；⚠️ ThumbnailService **非**多实例先例（借单 Pipeline + `RenderToTexture` 无后处理 + 帧外 WaitIdle），实时第二视口要第二完整 Pipeline（显存翻倍）+ M2 输入路由 + 游戏相机切换（M2 defer ③ 未落）。2026-07-07 复核维持 defer：无「不加第二 Pipeline 就能交付」的安全切片（真 Game 面板必须第二 render target + 游戏相机），硬接半版本会回归编辑器现行稳定轨道相机——待 M2 输入/相机路由落定后再做，dogfood 在场时验。M9 唯一未落项。
