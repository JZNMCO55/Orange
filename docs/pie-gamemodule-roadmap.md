...
- **实现进展（§①②③④，2026-07-07~08 多 session）**：
  ...
  - **剩**：① 真机 GUI dogfood（改手感代码重编 slime.dll → `[R]` 热重载生效）；② **game.dll schema 注册通道**（DLL 组件 Inspector authoring，与热重载正交的独立设计——SlimeTuningComponent 目前经 serializer round-trip、schema 靠 per-game SlimeEditor 的 onRegisterSchemas，共享 OrangeEditor 下 DLL 组件无 schema 通道）；③ 引擎 SHARED 化全量回归的 SHARED-config ctest（tests 白盒依赖内部符号，设计上 SHARED 跳过、跑 STATIC）。
