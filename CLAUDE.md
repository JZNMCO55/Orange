...
- Status (基准 2026-05-19)：**Phase 1 ~ 5.5 全 ✅** + **Phase 6.5 · 渲染真实感基线（PBR + IBL）渲染管线接通 + OE 端 follow-up 待补**（Task 06.5-01 ~ 06 全 ✅；06.5-07 sample 14_pbr_ibl 代码已落地，**segfault 真因（OR `mPipelineCache` dangling raw pointer，c12 锁定）已由 OR commit `5715a9d` fix + `4eec1bf` 归档，本仓 vendor bump 后 sample 跑通 init + frame loop**，但视觉验收暴露 2 个新 OE 端 GAP 阻塞 ✅：`docs/engine-known-gaps.md` GAP-2026-05-19-pbr-ibl-specular-quality + GAP-2026-05-19-editor-environment-component-wiring；两 GAP 落地后才标 Task 06.5-07 ✅ + Phase 6.5 整体 ✅；B.1 + B.2 acceptance-checklist 落 `docs/acceptance/`；samples 13_pbr_direct + 14_pbr_ibl 已编出）。现处 **Phase 6** —— OrangeEditor 工具链。OrangeEditor 自己按 semver 独立演进（v0.1 / v0.1.5 / v0.2 / v0.2.5 / v0.3 / v0.4 已 ✅；下一里程碑 **v0.5 Asset 浏览器 + Material 子模式**，见 `docs/editor-roadmap.md` 与 `docs/decisions/ADR-001`）。v0.8 编辑器伴随 milestone（Environment 浏览 / material thumbnail / sky placeholder）推到 Phase 6.5 整体 ✅ 后立项。`docs/design-plan.md` 的 ✅ 标记是 phase 进度的权威 source；本节描述若与之冲突，以 design-plan.md 为准。任何 commit 修改了 design-plan / editor-roadmap 的 ✅ 状态后，跑 `python scripts/check_claude_md_drift.py` 确认本节没有新漂移
