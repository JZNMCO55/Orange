...
namespace Orange
{
    ...
    static std::string RigidBody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
    {
        ...
        ORANGE_CORE_ASSERT(false, "Unknown body type");
        return {};
    }
    ...
    void SceneSerializer::Serialize(const std::string& filepath)
    {
        ...
        mpScene.lock()->GetRegistry().view<entt::entity>().each([&](auto entityId)
            {
                Entity entity = { entityId , mpScene };
                if (!entity)
                {
                    return;
                }
    ...
    bool SceneSerializer::Deserialize(const std::string& filepath)
    {
        ...
        ORANGE_LOG_INFO("Deserializing scene '{0}'", sceneName);
        ...
        if (entities)
        {
            ...
            {
                uint64_t uuid = entity["Entity"].as<uint64_t>(); // ToDo: Implement UUIDs
