...
namespace Orange
{
    namespace Utils
    {
        ...
        static shaderc_shader_kind GLShaderStageToShaderC(GLenum stage)
        {
            ...
            ORANGE_CORE_ASSERT(false, "Invalid shader stage specified");
        ...
        static const char* GetCacheDirectory()
        {
            ...
            return R"(../../Resource/Shaders/OpenGL)";
        }
    ...
    std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
    {
        ...
        while (pos != std::string::npos)
        {
            ...
            ORANGE_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
        ...
        for (auto&& [stage, source] : shaderSources)
        {
            ...
            else
            {
                shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, 
                    Utils::GLShaderStageToShaderC(stage), mFilePath.c_str(), options);

                if (module.GetCompilationStatus() != shaderc_compilation_status_success)
                {
                    ORANGE_LOG_ERROR(module.GetErrorMessage());
                    ORANGE_CORE_ASSERT(false, "Shader compilation failure!");
                }
    ...
    void OpenGLShader::CreateProgram()
    {
        ...
        if (isLinked == GL_FALSE)
        {
            ...
            ORANGE_LOG_ERROR("Shader linking failed ({0}):\n{1}", mFilePath, infoLog.data());
    ...
    void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderBinary)
    {
        spirv_cross::Compiler compiler(shaderBinary);
        spirv_cross::ShaderResources resources = compiler.get_shader_resources();

        ORANGE_LOG_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), mFilePath);
        ORANGE_LOG_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
        ORANGE_LOG_TRACE("    {0} resources", resources.sampled_images.size());

        ORANGE_LOG_TRACE("Uniform buffers:");
        for (const auto& resource : resources.uniform_buffers)
        {
            ...
            ORANGE_LOG_TRACE("  {0}", resource.name);
            ORANGE_LOG_TRACE("    Size = {0}", bufferSize);
            ORANGE_LOG_TRACE("    Binding = {0}", binding);
            ORANGE_LOG_TRACE("    Members = {0}", memberCount);
        }
    }
}