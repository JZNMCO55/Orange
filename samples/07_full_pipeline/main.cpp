...
#error "ORANGE_ENGINE_ACTIONS_DIR must be defined by CMake"
    ...
    public:
        GameLayer(In::InputContext&      input,
            ...
            : Layer("GameLayer"), mInput(input), mPhys(phys), mCharBody(charBody), mAnimator(animator), mWorld(world), mJointEntities(std::move(jointEntities)), mJumpSound(jumpSound)
        {
        }
...
int main(int argc, char** argv)
{
    ...
    const std::string skelPath   = std::string(ORANGE_ENGINE_SKELETON_DATA_DIR) + "/mecha_1406_ske.json";
    ...
    {
        const std::string actionsPath = std::string(ORANGE_ENGINE_ACTIONS_DIR) + "/default.actions.json";
