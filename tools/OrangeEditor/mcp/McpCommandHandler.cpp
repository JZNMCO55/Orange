...
namespace Orange::Editor::Mcp
{
    ...
    namespace
    {
        ...
        const char* PropertyTypeName(Schema::PropertyType t)
        {
            ...
            switch (t)
            {
                ...
                case PT::AssetRef:
                    return "AssetRef";
                case PT::AssetRefArray:
                    return "AssetRefArray";
                case PT::PolygonVertices:
                    return "PolygonVertices";
                case PT::EdgeChainVertices:
                    return "EdgeChainVertices";
            }
            return "Unknown";
        }
        ...
        const char* GizmoSpaceName(EditorGizmoState::Space sp)
        {
            switch (sp)
            {
                case EditorGizmoState::Space::World:
                    return "World";
                case EditorGizmoState::Space::Local:
                    return "Local";
            }
            return "World";
        }
                         ...
                         const void* comp, const EditorAssetContext& assets, entt::registry& reg)
        {
            ...
            switch (pd.type)
            {
                ...
                case PT::UInt:
                {
                    unsigned v = 0;
                    pd.get(comp, &v);
                    w.WriteInt(path, static_cast<std::int64_t>(v));
                    break;
                }
                case PT::Bool:
                {
                ...
                case PT::Quat:
                {
                    ...
                    pd.get(comp, &v);
                    const float a[4]{v.w, v.x, v.y, v.z};
                    w.WriteFloatArray(path, a, 4);
                    break;
                }
                case PT::String:
                ...
                case PT::EntityRef:
                {
                    Orange::Engine::Entity v;
                    pd.get(comp, &v);
                    w.WriteString(path, EntityGuidString(reg, v));
                    break;
                }
                case PT::AssetRef:
                {
                    std::string v;
                    if (pd.assetRefGet != nullptr)
                        pd.assetRefGet(comp, assets, &v);
                    w.WriteString(path, v);
                    break;
                }
                case PT::AssetRefArray:
                {
                    ...
                    for (std::size_t i = 0; i < v.size(); ++i)
                        w.WriteString(path + "/" + std::to_string(i), v[i]);
                    break;
                }
                case PT::PolygonVertices:
                {
                    ...
                    {
                        const float a[2]{d.vertices[i].x, d.vertices[i].y};
        ...
        std::string HandleGetSceneInfo(std::int64_t id, EditorHost& host)
        {
            ...
            if (pWorld == nullptr)
            {
                return MakeError(id, "no active world");
            }
            ...
            for (auto e : reg.view<entt::entity>())
            {
                if (records.size() >= kMaxSceneEntities)
                {
                    truncated = true;
                    break;
                }
        ...
        std::string HandleGetEntity(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (!entity.IsValid())
            {
                return MakeError(id, "entity not found");
            }
            ...
            for (const auto& sc : schemas)
            {
                ...
                for (const auto& pd : sc.properties)
                {
                    ...
                    if (pd.get == nullptr && pd.assetRefGet == nullptr)
                    {
                        continue;
                    }
        ...
        std::string HandleCaptureViewport(std::int64_t id, Orange::Engine::Render::Pipeline* pipeline)
        {
            if (pipeline == nullptr)
            {
                return MakeError(id, "viewport pipeline not ready");
            }
            ...
            if (w == 0 || h == 0 || w > kMaxCaptureDim || h > kMaxCaptureDim || bgra.size() != static_cast<std::size_t>(w) * static_cast<std::size_t>(h) * 4u)
            {
        ...
        const Schema::ComponentSchema* FindSchemaByName(const std::string& typeName)
        {
            for (const auto& sc : Schema::ComponentSchemaRegistry::Instance().All())
                if (sc.typeName != nullptr && typeName == sc.typeName)
                {
                    return &sc;
                }
            return nullptr;
        ...
        {
            if (host.scene.playState != PlayState::Play)
            {
                return false;
            }
                          ...
                          const std::string& fieldKey, const T& newVal)
        {
            ...
            if (pWorld != nullptr)
            {
                if (void* comp = sc.get(*pWorld, e); comp != nullptr)
                {
                    pd.get(comp, &oldVal);
                }
            }
            auto apply = [pHost = &host, e, pSchema = &sc, setFn = pd.set](const T& v)
            {
                ...
                if (w == nullptr || pSchema->get == nullptr || setFn == nullptr)
                {
                    return;
                }
                if (void* c = pSchema->get(*w, e); c != nullptr)
                {
                    setFn(c, &v);
                }
            };
                             ...
                             const std::string& fieldKey, const std::string& newVal)
        {
            ...
            if (pWorld != nullptr && pd.assetRefGet != nullptr)
            {
                if (void* comp = sc.get(*pWorld, e); comp != nullptr)
                {
                    pd.assetRefGet(comp, host.assets, &oldVal);
                }
            }
        ...
        std::string HandleSetField(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (!req.ReadString("args/component", compName))
            {
                return MakeError(id, "missing 'component'");
            }
            if (!req.ReadString("args/field", fieldName))
            {
                return MakeError(id, "missing 'field'");
            }
            if (!req.Has("args/value"))
            {
                return MakeError(id, "missing 'value'");
            }
            ...
            if (sc == nullptr)
            {
                return MakeError(id, "unknown component: " + compName);
            }
            if (sc->has == nullptr || !sc->has(*pWorld, e))
            {
                return MakeError(id, "component not on entity: " + compName);
            }

            const Schema::PropertyDescriptor* pd = nullptr;
            for (const auto& p : sc->properties)
                if (p.name != nullptr && fieldName == p.name)
                {
                    pd = &p;
                    break;
                }
            if (pd == nullptr)
            {
                return MakeError(id, "unknown field: " + fieldName);
            }
            ...
            switch (pd->type)
            {
                ...
                {
                    double d;
                    if (!req.ReadFloat(vp, d))
                        return MakeError(id, "value must be a number");
                    PushFieldSet<float>(host, e, *sc, *pd, fieldKey, ClampRange(static_cast<float>(d), pd->attribs));
                    break;
                }
                case PT::Int:
                {
                    std::int64_t i;
                    if (!req.ReadInt(vp, i))
                        return MakeError(id, "value must be an integer");
                    if (pd->attribs.hasRange)
                        i = static_cast<std::int64_t>(ClampRange(static_cast<float>(i), pd->attribs));
                    PushFieldSet<int>(host, e, *sc, *pd, fieldKey, static_cast<int>(i));
                    break;
                }
                case PT::UInt:
                {
                    ...
                    if (pd->attribs.hasRange)
                        i = static_cast<std::int64_t>(ClampRange(static_cast<float>(i), pd->attribs));
                    PushFieldSet<unsigned>(host, e, *sc, *pd, fieldKey, static_cast<unsigned>(i));
                    break;
                }
                case PT::Bool:
                {
                    bool b;
                    if (!req.ReadBool(vp, b))
                        return MakeError(id, "value must be a bool");
                    PushFieldSet<bool>(host, e, *sc, *pd, fieldKey, b);
                    break;
                }
                case PT::Vec2:
                {
                    float a[2];
                    if (!req.ReadFloatArray(vp, a, 2))
                        return MakeError(id, "value must be a 2-number array");
                    PushFieldSet<glm::vec2>(host, e, *sc, *pd, fieldKey, glm::vec2(a[0], a[1]));
                    break;
                }
                case PT::Vec3:
                {
                    float a[3];
                    if (!req.ReadFloatArray(vp, a, 3))
                        return MakeError(id, "value must be a 3-number array");
                    PushFieldSet<glm::vec3>(host, e, *sc, *pd, fieldKey, glm::vec3(a[0], a[1], a[2]));
                    break;
                }
                case PT::Vec4:
                {
                    float a[4];
                    if (!req.ReadFloatArray(vp, a, 4))
                        return MakeError(id, "value must be a 4-number array");
                    PushFieldSet<glm::vec4>(host, e, *sc, *pd, fieldKey, glm::vec4(a[0], a[1], a[2], a[3]));
                    break;
                }
                case PT::Quat:
                {
                    float a[4];
                    if (!req.ReadFloatArray(vp, a, 4))
                        return MakeError(id, "value must be a 4-number array [w,x,y,z]");
                    PushFieldSet<glm::quat>(host, e, *sc, *pd, fieldKey, glm::quat(a[0], a[1], a[2], a[3]));
                    break;
                }
                case PT::String:
                {
                    std::string s;
                    if (!req.ReadString(vp, s))
                        return MakeError(id, "value must be a string");
                    PushFieldSet<std::string>(host, e, *sc, *pd, fieldKey, s);
                    break;
                }
                case PT::Enum:
                {
                    ...
                    if (req.ReadString(vp, name))
                    {
                        if (pd->attribs.enumNames != nullptr)
                            for (int k = 0; k < pd->attribs.enumCount; ++k)
                                if (name == pd->attribs.enumNames[k])
                                {
                                    idx = k;
                                    break;
                                }
                        if (idx < 0)
                            return MakeError(id, "invalid enum name: " + name);
                    }
                    else
                    {
                        std::int64_t i;
                        if (!req.ReadInt(vp, i))
                            return MakeError(id, "value must be enum name or int");
                        idx = static_cast<int>(i);
                    }
                    if (idx < 0 || (pd->attribs.enumCount > 0 && idx >= pd->attribs.enumCount))
                        return MakeError(id, "enum value out of range");
                    PushFieldSet<int>(host, e, *sc, *pd, fieldKey, idx);
                    break;
                }
                case PT::AssetRef:
                {
                    std::string s;
                    if (!req.ReadString(vp, s))
                        return MakeError(id, "value must be an asset path string");
                    if (pd->assetRefSet == nullptr)
                        return MakeError(id, "asset field not writable");
        ...
        std::string HandleCreateEntity(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (IsPlayBlocked(host, req))
            {
                return MakeError(id, "in play mode (pass allowInPlay:true to override)");
            }
            ...
            {
                Guid pg;
                if (!Guid::FromString(parentGuidStr, pg))
                {
                    return MakeError(id, "invalid parentGuid format");
                }
                Scene::EnsureEntityGuids(*pWorld);
                parent = Scene::FindEntityByGuid(*pWorld, pg);
                if (!parent.IsValid())
                {
                    return MakeError(id, "parent entity not found");
                }
            }
            ...
            if (!created.IsValid())
            {
                return MakeError(id, "create failed");
            }
        ...
        std::string HandleAddComponent(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (sc->add == nullptr)
            {
                return MakeError(id, "component not addable: " + compName);
            }
            if (sc->has != nullptr && sc->has(*pWorld, e))
            {
                return MakeError(id, "component already present: " + compName);
            }
        ...
        std::string HandleFindEntities(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            {
                Guid ug;
                if (!Guid::FromString(underGuidStr, ug))
                {
                    return MakeError(id, "invalid underGuid format");
                }
                under = Scene::FindEntityByGuid(*pWorld, ug);
                if (!under.IsValid())
                {
                    return MakeError(id, "underGuid entity not found");
                }
            }
            ...
            for (auto e : reg.view<entt::entity>())
            {
                if (hits.size() >= kMaxSceneEntities)
                {
                    truncated = true;
                    break;
                }
                ...
                if (!nameSubLower.empty() && AsciiLower(nm).find(nameSubLower) == std::string::npos)
                {
                    continue;
                }
                if (compSchema != nullptr &&
                    (compSchema->has == nullptr || !compSchema->has(*pWorld, entity)))
                {
                    continue;
                }
                // under 是 entity 的祖先（含自身）== entity 在 under 的子树内。
                if (under.IsValid() && !EditorHierarchy::IsAncestorOf(*pWorld, under, entity))
                {
                    continue;
                }
        ...
        std::string HandleSetCamera(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            auto& ec = host.camera;
            float p[3];
            if (req.ReadFloatArray("args/pivot", p, 3))
            {
                ec.pivot = glm::vec3(p[0], p[1], p[2]);
            }
            double d;
            if (req.ReadFloat("args/azimuth", d))
            {
                ec.azimuth = static_cast<float>(d);
            }
            ...
            if (req.ReadFloat("args/radius", d))
            {
                ec.radius = std::max(static_cast<float>(d), 0.01f);
            }
            if (req.ReadFloat("args/fovYDegrees", d))
            {
                ec.fovYDegrees = std::min(std::max(static_cast<float>(d), 1.0f), 179.0f);
            }
            if (req.ReadFloat("args/zNear", d))
            {
                ec.zNear = std::max(static_cast<float>(d), 1e-4f);
            }
            if (req.ReadFloat("args/zFar", d))
            {
                ec.zFar = std::max(static_cast<float>(d), ec.zNear + 1e-3f);
            }
        ...
        std::string HandleFrameEntity(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (!ok)
            {
                return MakeError(id, "frame failed (empty scene / entity lacks transform)");
            }
        ...
        std::string HandleDuplicateEntity(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (blobRes.IsErr())
            {
                return MakeError(id, "subtree serialize failed");
            }
            ...
            host.cmdStack.Push(std::make_unique<LambdaCommand>(
                ...
                [pH, blob, origParent, createdPtr]()
                {
                    ...
                    if (Scene::LoadFromString(blob, *w, lo, &created).IsErr())
                    {
                        return;
                    }
                    *createdPtr = created;
        ...
        std::string HandleReparentEntity(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            {
                Guid np;
                if (!Guid::FromString(npStr, np))
                {
                    return MakeError(id, "invalid newParentGuid format");
                }
                newParent = Scene::FindEntityByGuid(*pW, np);
                if (!newParent.IsValid())
                {
                    return MakeError(id, "new parent entity not found");
                }
            }
            ...
            host.cmdStack.Push(std::make_unique<LambdaCommand>(
                ...
                {
                    auto* w = pH->scene.pWorld.get();
                    if (w == nullptr || !w->IsValid(child))
                    {
                        return;
                    }
                    if (keepWorld)
                    {
                        EditorHierarchy::ReparentToKeepWorld(*w, child, newParent);
                    }
                    else
                    {
                        EditorHierarchy::ReparentTo(*w, child, newParent);
                    }
                },
                [pH, child, oldParent, oldPrev, keepWorld]()
                {
                    ...
                    if (keepWorld)
                    {
                        EditorHierarchy::MoveToPositionKeepWorld(*w, child, oldParent, oldPrev);
                    }
                    else
                    {
                        EditorHierarchy::MoveToPosition(*w, child, oldParent, oldPrev);
                    }
                }));
        ...
        std::string HandleRemoveComponent(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (sc->remove == nullptr)
            {
                return MakeError(id, "component not removable: " + compName);
            }
            if (sc->has == nullptr || !sc->has(*pW, e))
            {
                return MakeError(id, "component not on entity: " + compName);
            }
            ...
            if (sc->add != nullptr && comp != nullptr)
            {
                ...
                host.cmdStack.Push(std::make_unique<LambdaCommand>(
                    ...
                    [pH, e, sc]()
                    {
                        if (pH->scene.pWorld && sc->remove)
                        {
                            sc->remove(*pH->scene.pWorld, e);
                        }
                    },
                    [pH, e, sc, restorers]()
                    {
        ...
        std::string HandleBeginUndoGroup(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            {
                return MakeError(id, "undo group already open");
            }
        ...
        std::string HandleOpenScene(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (!req.ReadString("args/path", path) || path.empty())
            {
                return MakeError(id, "missing 'path'");
            }
            if (host.scene.playState != PlayState::Edit)
            {
                return MakeError(id, "cannot open scene while in play mode");
            }

            std::error_code ec;
            if (!fs::exists(path, ec))
            {
                return MakeError(id, "scene file not found: " + path);
            }
        ...
        std::string HandleSetPlayOp(std::int64_t id, EditorHost& host, const std::string& opName)
        {
            ...
            if (opName == "play")
            {
                if (st != PlayState::Edit)
                {
                    return MakeError(id, "already in play/paused");
                }
                op = PlayOp::EnterPlay;
            }
            else if (opName == "pause")
            {
                if (st != PlayState::Play)
                {
                    return MakeError(id, "not in play (pause requires Play)");
                }
                op = PlayOp::Pause;
            }
            else if (opName == "resume")
            {
                ...
                {
                    return MakeError(id, "not paused (resume requires Paused)");
                }
                op = PlayOp::Resume;
            }
            else if (opName == "stop")
            {
                if (st == PlayState::Edit)
                {
                    return MakeError(id, "already in edit");
                }
                op = PlayOp::Stop;
            }
            else
        ...
        const char* AssetKindByPath(const std::filesystem::path& p)
        {
            ...
            if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".tga" || ext == ".ktx" || ext == ".hdr" || ext == ".exr" || ext == ".texture")
            {
                return "Texture";
            }
        ...
        std::string HandleListAssets(std::int64_t id, const JsonReader& req)
        {
            ...
            if (fs::exists(root, ec) && fs::is_directory(root, ec))
            {
                     ...
                     it != fs::recursive_directory_iterator(); it.increment(ec))
                {
                    ...
                    if (!kindFilter.empty() && kindFilter != kind)
                    {
                        continue;
                    }
                    std::string rel = it->path().generic_string(); // forward slashes
                    if (!prefix.empty() && rel.find(prefix) == std::string::npos)
                    {
                        continue;
                    }
                    assets.push_back(A{std::move(rel), kind});
                    if (assets.size() >= kMaxAssets)
                    {
                        truncated = true;
                        break;
                    }
        ...
        std::string HandleImportAsset(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            namespace fs = std::filesystem;
            std::string src;
            if (!req.ReadString("args/srcPath", src) || src.empty())
            {
                return MakeError(id, "missing 'srcPath'");
            }
            std::error_code ec;
            if (!fs::exists(src, ec))
            {
                return MakeError(id, "source file not found: " + src);
            }
        ...
        std::string HandleSetEntityOrder(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            req.ReadString("args/guid", guidStr);
            if (!req.ReadString("args/direction", dir))
            {
                return MakeError(id, "missing 'direction' (up|down)");
            }
            int delta = 0;
            if (dir == "up")
            {
            ...
            {
                return MakeError(id, "direction must be 'up' or 'down'");
            }
        ...
        std::string HandleNewScene(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            if (host.scene.playState != PlayState::Edit)
            {
                return MakeError(id, "cannot new scene while in play mode");
            }
            const bool force = req.GetBool("args/force", false);
            if (host.scene.dirty && !force)
            {
        ...
        std::string HandleInstantiatePrefab(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (pReg == nullptr)
            {
                return MakeError(id, "AssetRegistry not ready");
            }
            ...
            if (loaded.IsErr())
            {
                return MakeError(id, "load prefab failed: " + path);
            }
            ...
            if (!root.IsValid())
            {
                return MakeError(id, "instantiate failed");
            }
        ...
        std::string HandleGetPrefabStatus(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (pi == nullptr)
            {
                return MakeError(id, "no PrefabInstanceComponent");
            }
        ...
        std::string HandleRevertOverride(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (!ok)
            {
                return MakeError(id, "revert no-op (field not overridden / no template / no overrides)");
            }
        ...
        std::string HandleGetAnimationClip(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (!guidStr.empty())
            {
                ...
                if (clip == nullptr)
                {
                    return MakeError(id, "entity has no ClipAnimator");
                }
            ...
            else if (!path.empty())
            {
                ...
                if (loaded.IsErr())
                {
                    return MakeError(id, "load .anim failed: " + path);
                }
        ...
        std::string HandleSetAnimationClip(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (parsed.IsErr())
            {
                return MakeError(id, "invalid clip JSON (schema/key order/interp)");
            }
        ...
        std::string HandlePreviewAnimation(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            req.ReadString("args/guid", guidStr);
            if (!req.ReadString("args/action", action))
            {
                return MakeError(id, "missing 'action' (play|pause|seek)");
            }
            std::string  err;
            const Entity e = ResolveEntityByGuid(host, guidStr, err);
            if (!e.IsValid())
            ...
            {
                double t = 0.0;
                if (!req.ReadFloat("args/time", t))
                {
                    return MakeError(id, "seek requires 'time' (seconds)");
                }
                host.animPreview.previewEntity  = e;
                host.animPreview.previewPlaying = false;
        ...
        std::string HandleSetScriptField(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            req.ReadString("args/guid", guidStr);
            if (!req.ReadString("args/fieldName", fieldName) || fieldName.empty())
            {
                return MakeError(id, "missing 'fieldName'");
            }
            if (!req.Has("args/value"))
            {
                return MakeError(id, "missing 'value'");
            ...
            if (sc == nullptr)
            {
                return MakeError(id, "entity has no ScriptComponent");
            }
            ...
            if (hasExplicitType)
            {
                if (typeStr == "Float")
                {
                    ftype = Script::ScriptFieldType::Float;
                }
                else if (typeStr == "Int")
                {
                    ftype = Script::ScriptFieldType::Int;
                }
                else if (typeStr == "Bool")
                {
                    ftype = Script::ScriptFieldType::Bool;
                }
                else if (typeStr == "String")
                {
                    ftype = Script::ScriptFieldType::String;
                }
                else
                {
                    return MakeError(id, "type must be Float|Int|Bool|String");
                }
            }
            ...
            if (hasExplicitType)
            {
                switch (ftype)
                {
                    ...
                    {
                        if (!req.ReadString("args/value", valueStr))
                            return MakeError(id, "value must be a string");
                        break;
                    }
                }
            ...
            else
            {
                ...
                else if (req.ReadString("args/value", s))
                {
                    ftype    = Script::ScriptFieldType::String;
                    valueStr = s;
                }
                else if (req.ReadInt("args/value", i))
                {
                    ftype    = Script::ScriptFieldType::Int;
                    valueStr = std::to_string(i);
                }
                else if (req.ReadFloat("args/value", d))
                {
                    ftype    = Script::ScriptFieldType::Float;
                    valueStr = std::to_string(d);
                }
                ...
                {
                    return MakeError(id, "value must be a number / bool / string");
                }
            }
        ...
        std::string HandleSetMaterialParam(std::int64_t id, EditorHost& host, const JsonReader& req)
        {
            ...
            if (!req.ReadString("args/param", param) || param.empty())
            {
                return MakeError(id, "missing 'param'");
            }
            if (!req.Has("args/value"))
            {
                return MakeError(id, "missing 'value'");
            ...
            if (!loaded)
            {
                return MakeError(id, "read material failed (missing / bad schema): " + path);
            }
            ...
            switch (target->type)
            {
                case MUT::Float:
                {
                    ...
                    if (!req.ReadFloat("args/value", d))
                        return MakeError(id, "value must be a number");
                    target->value = static_cast<float>(d);
                    break;
                }
                case MUT::Int:
                {
                    ...
                    if (!req.ReadInt("args/value", i))
                        return MakeError(id, "value must be an integer");
                    target->value = static_cast<std::int32_t>(i);
                    break;
                }
                case MUT::Vec2:
                {
                    float a[2];
                    if (!req.ReadFloatArray("args/value", a, 2))
                        return MakeError(id, "value must be a 2-number array");
                    target->value = glm::vec2(a[0], a[1]);
                    break;
                }
                case MUT::Vec3:
                {
                    float a[3];
                    if (!req.ReadFloatArray("args/value", a, 3))
                        return MakeError(id, "value must be a 3-number array");
                    target->value = glm::vec3(a[0], a[1], a[2]);
                    break;
                }
                case MUT::Vec4:
                {
                    float a[4];
                    if (!req.ReadFloatArray("args/value", a, 4))
                        return MakeError(id, "value must be a 4-number array");
                    target->value = glm::vec4(a[0], a[1], a[2], a[3]);
                    break;
                }
                case MUT::Mat4:
                default:
        ...
        std::string HandleGetEditorLog(std::int64_t id, const JsonReader& req, const McpLogReader& logReader)
        {
            ...
            if (logReader)
            {
                entries = logReader(lines > 0 ? lines : 100, minLevel);
            }
                                  ...
                                  const McpLogReader&               logReader)
    {
        ...
        try
        {
            ...
            if (op == "ping")
            {
                return HandlePing(id, host);
            }
            if (op == "get_scene_info")
            {
                return HandleGetSceneInfo(id, host);
            }
            if (op == "get_entity")
            {
                return HandleGetEntity(id, host, r);
            }
            if (op == "get_editor_state")
            {
                return HandleGetEditorState(id, host);
            }
            if (op == "list_component_types")
            {
                return HandleListComponentTypes(id);
            }
            if (op == "capture_viewport")
            {
                return HandleCaptureViewport(id, viewportPipeline);
            }
            if (op == "set_field")
            {
                return HandleSetField(id, host, r);
            }
            if (op == "create_entity")
            {
                return HandleCreateEntity(id, host, r);
            }
            if (op == "add_component")
            {
                return HandleAddComponent(id, host, r);
            }
            if (op == "delete_entity")
            {
                return HandleDeleteEntity(id, host, r);
            }
            if (op == "select_entity")
            {
                return HandleSelectEntity(id, host, r);
            }
            if (op == "save_scene")
            {
                return HandleSaveScene(id, host, r);
            }

            // ---- P1 ----
            if (op == "find_entities")
            {
                return HandleFindEntities(id, host, r);
            }
            if (op == "get_camera")
            {
                return HandleGetCamera(id, host);
            }
            if (op == "set_camera")
            {
                return HandleSetCamera(id, host, r);
            }
            if (op == "frame_entity")
            {
                return HandleFrameEntity(id, host, r);
            }
            if (op == "duplicate_entity")
            {
                return HandleDuplicateEntity(id, host, r);
            }
            if (op == "reparent_entity")
            {
                return HandleReparentEntity(id, host, r);
            }
            if (op == "remove_component")
            {
                return HandleRemoveComponent(id, host, r);
            }
            if (op == "begin_undo_group")
            {
                return HandleBeginUndoGroup(id, host, r);
            }
            if (op == "end_undo_group")
            {
                return HandleEndUndoGroup(id, host);
            }
            if (op == "open_scene")
            {
                return HandleOpenScene(id, host, r);
            }
            ...
            if (op == "list_assets")
            {
                return HandleListAssets(id, r);
            }
            if (op == "import_asset")
            {
                return HandleImportAsset(id, host, r);
            }

            // ---- P2 ----
            if (op == "set_entity_order")
            {
                return HandleSetEntityOrder(id, host, r);
            }
            if (op == "new_scene")
            {
                return HandleNewScene(id, host, r);
            }
            if (op == "create_prefab")
            {
                return HandleCreatePrefab(id, host, r);
            }
            if (op == "instantiate_prefab")
            {
                return HandleInstantiatePrefab(id, host, r);
            }
            if (op == "get_prefab_status")
            {
                return HandleGetPrefabStatus(id, host, r);
            }
            if (op == "revert_override")
            {
                return HandleRevertOverride(id, host, r);
            }
            if (op == "apply_instance")
            {
                return HandleApplyInstance(id, host, r);
            }
            if (op == "get_animation_clip")
            {
                return HandleGetAnimationClip(id, host, r);
            }
            if (op == "set_animation_clip")
            {
                return HandleSetAnimationClip(id, host, r);
            }
            if (op == "preview_animation")
            {
                return HandlePreviewAnimation(id, host, r);
            }
            if (op == "set_script_field")
            {
                return HandleSetScriptField(id, host, r);
            }
            if (op == "create_material")
            {
                return HandleCreateMaterial(id, host, r);
            }
            if (op == "set_material_param")
            {
                return HandleSetMaterialParam(id, host, r);
            }
            if (op == "get_editor_log")
            {
                return HandleGetEditorLog(id, r, logReader);
            }
    ...
    void TickMcpUndoGroupGuard(EditorHost& host)
    {
        ...
        const bool disconnected = !bridge.clientConnected.load() || bridge.clientGeneration.load() != bridge.undoGroupClientGen;
