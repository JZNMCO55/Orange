...
namespace
{
    ...
    // 从 argv 解析可选 `--project <path.orangeproject>`（M6 工程模型）。缺省返回空串
    //（走原版编辑器路径）。命中即返回清单文件路径，由 RunEditorApp 加载填 EditorAppConfig。
    std::string ParseProjectFlag(int argc, char** argv)
    {
        for (int i = 1; i + 1 < argc; ++i)
        {
...
int Orange::Editor::RunEditorApp(int argc, char** argv, EditorAppConfig config)
{
    ...
    // M6 工程模型：`--project <path.orangeproject>` 命中即解析清单填 EditorAppConfig
    //（projectRoot / startupScene / windowTitle）——**必须在下面 chdir 读 config.projectRoot
    // 之前**。给原版 OrangeEditor 用；per-game editor 走编译期 config 注入不传本 flag。
    // 只填空字段，坏清单回退原版路径（不崩）。
    {
