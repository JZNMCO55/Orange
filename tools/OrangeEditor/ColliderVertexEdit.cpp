...
namespace Orange::Editor
{
    namespace
    {
                      ...
                      std::uint32_t minVertices, bool closedLoop)
        {
            ...
            if (cs.dragging && cs.selectedVertex >= 0 && cs.selectedVertex < static_cast<int>(cur.count))
            {
                    ...
                    if (mouseToLocal(newLocal))
                    {
                        ...
                        if (cur.count >= 2)
                        {
                            ...
                            for (std::uint32_t i = 0; i < edgeCount; ++i)
                            {
                                ...
                                if (!screen[i].valid || !screen[j].valid)
                                {
                                    continue;
                                }
                                const float d =
            ...
            else if (inViewport && hover >= 0 && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) && cur.count > minVertices)
            {
            ...
            for (std::uint32_t i = 0; i < cur.count; ++i)
            {
                ...
                if (static_cast<int>(i) == cs.selectedVertex && cs.dragging)
                {
                    col = kColSelected;
                }
                else if (static_cast<int>(i) == hover)
                {
                    col = kColHover;
                }
