...
namespace
{
    ...
    {
        if (auto* p = world.GetComponent<HC>(e))
        {
            return *p;
        }
...
namespace EditorHierarchy
{
    ...
    void LinkAsLastChild(World& world, Entity parent, Entity child)
    {
        ...
        for (std::size_t guard = 0; guard < kMaxSiblingWalk; ++guard)
        {
            ...
            if (h == nullptr || !h->nextSibling.IsValid())
            {
                break;
            }
            cur = h->nextSibling;
    ...
    void Detach(World& world, Entity e)
    {
        ...
        if (prev.IsValid())
        {
            if (HC* ph = world.GetComponent<HC>(prev))
            {
                ph->nextSibling = next;
            }
        }
        else
        {
            ...
            if (HC* pp = world.GetComponent<HC>(parent))
            {
                pp->firstChild = next;
            }
        }
        if (next.IsValid())
        {
            if (HC* nh = world.GetComponent<HC>(next))
            {
                nh->prevSibling = prev;
            }
        }
    ...
    void MoveToPosition(World& world, Entity child, Entity parent, Entity afterSibling)
    {
        ...
        if (!afterSibling.IsValid())
        {
            ...
            if (oldFirst.IsValid())
            {
                if (HC* oh = world.GetComponent<HC>(oldFirst))
                {
                    oh->prevSibling = child;
                }
            }
            return;
        }
    ...
    bool MoveRootRelative(World& world, Entity root, int delta, bool dryRun)
    {
        ...
        if (rh != nullptr && rh->parent.IsValid())
        {
            return false;
        }
        ...
        if (static_cast<std::size_t>(target) == idx)
        {
            return false;
        } // 已在边界
