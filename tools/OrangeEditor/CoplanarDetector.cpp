...
namespace Orange::Editor::Coplanar
{
    namespace
    {
        ...
        FacesOverlap(const LocalAABB& a, const LocalAABB& b, Face axisFace) noexcept
        {
            ...
            switch (axisFace)
            {
                case Face::Right:
                case Face::Left:
                    return intervalsOverlap(a.min.y, a.max.y, b.min.y, b.max.y) && intervalsOverlap(a.min.z, a.max.z, b.min.z, b.max.z);
                case Face::Top:
                case Face::Bottom:
                    return intervalsOverlap(a.min.x, a.max.x, b.min.x, b.max.x) && intervalsOverlap(a.min.z, a.max.z, b.min.z, b.max.z);
                case Face::Front:
                case Face::Back:
                    return intervalsOverlap(a.min.x, a.max.x, b.min.x, b.max.x) && intervalsOverlap(a.min.y, a.max.y, b.min.y, b.max.y);
            }
            return false;
        }
                   ...
                   float                  maxDist)
    {
        ...
        if (pSelfXform == nullptr || pSelfRc == nullptr)
        {
            return hits;
        }
        ...
        for (auto e : view)
        {
            ...
            if (glm::length(centerDelta) > maxDist + glm::length(selfBox.halfExtents) + glm::length(otherBox.halfExtents))
            {
                continue;
            }
            ...
            for (Face selfFace : kAllFaces)
            {
                ...
                if (!FacesOverlap(selfBox.aabb, otherBox.aabb, selfFace))
                {
                    continue;
                }
