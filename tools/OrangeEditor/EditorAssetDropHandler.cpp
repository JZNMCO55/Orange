...
namespace Orange::Editor
{
    namespace
    {
                           ...
                           const std::string&     path)
        {
            ...
            if (host.scene.pWorld == nullptr || !host.scene.pWorld->IsValid(target))
            {
                                     ...
                                     const std::string&     meshPath)
    {
        ...
        if (pW == nullptr || !pW->IsValid(entity))
        {
            return;
        }
    ...
    {
        if (assetPath.empty() || !target.IsValid())
        {
            return false;
        }
        const std::string ext = GetExtension(assetPath);
        if (IsMaterialExt(ext))
        {
            return ApplyMaterial(host, target, assetPath);
        }
        if (IsMeshExt(ext))
        {
            return ApplyMesh(host, target, assetPath);
        }
        if (IsAudioExt(ext))
        {
            return ApplySound(host, target, assetPath);
        }
