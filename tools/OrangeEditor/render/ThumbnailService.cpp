...
namespace Orange::Editor::Render
{
    ...
    ThumbnailService::ThumbnailService(EditorHost&                     host,
        ...
        : mHost(host), mDevice(device), mImguiDescriptorPool(imguiDescriptorPool)
    {
        CreateSampler();
    }
    ...
    bool ThumbnailService::IsPending(const std::string& path) const
    {
        return std::find_if(mPending.begin(), mPending.end(),
                            [&](const PendingItem& p)
                            { return p.path == path; }) != mPending.end();
    }
                                         ...
                                         std::uint64_t frameIndex)
    {
        ...
        switch (kind)
        {
            case ThumbKind::Material:
                return BakeMaterialThumbnail(path, frameIndex);
            case ThumbKind::Prefab:
                return BakePrefabThumbnail(path, frameIndex);
            case ThumbKind::Mesh:
                return BakeMeshThumbnail(path, frameIndex);
            case ThumbKind::Scene:
                return BakeSceneThumbnail(path, frameIndex);
        }
        return false;
    }
                                        ...
                                        ThumbKind              kind)
    {
        ...
        if (entry.pRt == nullptr)
        {
            ...
            td.mUsage  = Orange::Rhi::TextureUsage::RenderTarget | Orange::Rhi::TextureUsage::Sampled;
                                              ...
                                              std::uint64_t      frameIndex)
    {
        ...
        if (!ReadFileToString(scenePath, sceneBlob))
        {
            return false;
        }
        ...
        struct SceneGuard
        {
            ...
            ~SceneGuard()
            {
                if (pWorld == nullptr || pCreated == nullptr)
                {
                    return;
                }
    ...
    void ThumbnailService::CreateSampler()
    {
        ...
        s.minLod       = 0.0f;
        s.maxLod       = 0.0f;
        if (pfnCreate(static_cast<VkDevice>(handles.vkDevice), &s, nullptr, &mSampler) != VK_SUCCESS)
        {
