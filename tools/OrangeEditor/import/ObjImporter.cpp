...
namespace Orange::Editor::Import
{
                                        ...
                                        ::Orange::Engine::Asset::AssetRegistry& registry)
    {
        ...
        {
            totalFaceVerts += sh.mesh.indices.size();
        }
            ...
            for (std::size_t tri = 0; tri < triCount; ++tri)
            {
                ...
                for (int k = 0; k < 3; ++k)
                {
                    ...
                    if (it == dedup.end())
                    {
                        ...
                        VertexPosition3 p{};
                        if (fv.vertex_index >= 0 && static_cast<std::size_t>(fv.vertex_index) * 3 + 2 < attrib.vertices.size())
                        {
                        ...
                        VertexUV2 uv{};
                        if (fv.texcoord_index >= 0 && static_cast<std::size_t>(fv.texcoord_index) * 2 + 1 < attrib.texcoords.size())
                        {
                        ...
                        VertexNormal3 nrm{};
                        if (fv.normal_index >= 0 && static_cast<std::size_t>(fv.normal_index) * 3 + 2 < attrib.normals.size())
                        {
        ...
        if (multiMaterial)
        {
            ...
            for (std::uint32_t slot = 0; slot < orderedObjMat.size(); ++slot)
            {
                ...
                if (count > 0)
                {
                    subMeshes.push_back({offset, count, slot});
                }
            }
        }
        else
        ...
        {
            std::string name = (objMat >= 0 && static_cast<std::size_t>(objMat) < materials.size() && !materials[objMat].name.empty())
