...
{
    return (*ud->indices)[static_cast<std::size_t>(iFace) * 3u
                          + static_cast<std::size_t>(iVert)];
}
                    ...
                    const int iFace, const int iVert)
{
    ...
    ud->faceVertTangents[static_cast<std::size_t>(iFace) * 3u
                         + static_cast<std::size_t>(iVert)] =
    ...
    std::vector<VertexTangent4>&  outTangents)
{
    ...
    if (uvs.size() != positions.size() || normals.size() != positions.size()) { return false; }
    ...
    if (genTangSpaceDefault(&ctx) == 0) { return false; }
    ...
    for (std::size_t fv = 0; fv < indices.size(); ++fv)
    {
        ...
        {
            if (TangentApproxEqual(cand.first, t)) { mapped = cand.second; break; }
        }
        if (mapped == kNone)
        {
