...
namespace Orange::Editor::Schema
{
    ...
    namespace
    {
                                  ...
                                  bool canPaste = false)
        {
            ...
            const bool wantMenu = (removable && outRemove != nullptr) || outCopy != nullptr || outPaste != nullptr;
            if (wantMenu && ImGui::BeginPopupContextItem(label))
            {
                ...
                if (outPaste != nullptr && ImGui::MenuItem("Paste Values", nullptr, false, canPaste))
                {
                    *outPaste = true;
                }
                if (removable && outRemove != nullptr)
                {
                    if (outCopy != nullptr || outPaste != nullptr)
                    {
                        ImGui::Separator();
                    }
                            ...
                            PropertyDescriptor::SetFn setFn)
        {
            ...
            {
                if (pHost == nullptr || pSchema == nullptr || pSchema->get == nullptr || setFn == nullptr)
                {
                    return;
                }
                                       ...
                                       const T&                  newVal)
        {
            ...
            if (schema.get == nullptr || prop.get == nullptr || prop.set == nullptr)
            {
                return;
            }
        ...
        {
            if (!host.selection.additionalSelectedEntities.empty() && !host.cmdStack.InGroup())
            {
            ...
            for (const auto& prop : schema.properties)
            {
                ...
                if (prop.assetRefGet != nullptr && prop.assetRefSet != nullptr)
                {
                    ...
                    if (prop.type == PropertyType::AssetRefArray)
                    {
                        ...
                        restorers.push_back([v, setFn, pH](void* c)
                                            { setFn(c, pH->assets, &v); });
                    }
                    else
                    {
                ...
                if (prop.get == nullptr || prop.set == nullptr)
                {
                    continue;
                }
                ...
                switch (prop.type)
                {
                    ...
                    {
                        float v{};
                        prop.get(component, &v);
                        restorers.push_back([v, setFn](void* c)
                                            { setFn(c, &v); });
                        break;
                    }
                    ...
                    {
                        Orange::Engine::Physics::PolygonDesc v{};
                        prop.get(component, &v);
                        restorers.push_back([v, setFn](void* c)
                                            { setFn(c, &v); });
                        break;
                    ...
                    {
                        Orange::Engine::Physics::EdgeChainDesc v{};
                        prop.get(component, &v);
                        restorers.push_back([v, setFn](void* c)
                                            { setFn(c, &v); });
                        break;
            ...
            switch (prop.type)
            {
                ...
                case PropertyType::UInt:
                {
                    ...
                    const unsigned int minV   = prop.attribs.hasRange
                                                    ? static_cast<unsigned int>(prop.attribs.minValue)
                                                    : 0u;
                    const unsigned int maxV   = prop.attribs.hasRange
                                                    ? static_cast<unsigned int>(prop.attribs.maxValue)
                                                    : 0u;
                ...
                case PropertyType::Quat:
                {
                                        ...
                                        dragSpeed))
                    {
                        ...
                        host.cmdStack.Push(std::make_unique<SetFieldValueCommand<glm::quat>>(
                            entity, fieldKey, oldVal, newVal,
                            [pHost, entity, pSchema, setFn](const glm::quat& v)
                            {
                                if (pHost == nullptr || pSchema == nullptr || pSchema->get == nullptr || setFn == nullptr)
                                {
                                    return;
                ...
                case PropertyType::AssetRef:
                {
                    ...
                    const bool useCtxAccessor = (prop.assetRefGet != nullptr) || (prop.assetRefSet != nullptr);
                    ...
                    auto makeApply = [&]() -> SetFieldValueCommand<std::string>::ApplyFn
                    {
                        ...
                        if (prop.attribs.assetKind == AssetKind::Mesh && useCtxAccessor)
                        {
                            ...
                            {
                                if (pH == nullptr || pSchema == nullptr || pSchema->get == nullptr || setFn == nullptr)
                                {
                                    return;
                                }
                    ...
                    const float       btnXW    = ImGui::CalcTextSize("\xC3\x97").x + s.FramePadding.x * 2.0f; // "×"
                    const float       btnPickW = ImGui::CalcTextSize("Pick").x + s.FramePadding.x * 2.0f;
                    ...
                    const bool         pickEnabled = !browserSel.empty() && browserSel != oldPath && canWrite;
                    ImGui::SameLine();
                ...
                case PropertyType::AssetRefArray:
                {
                    ...
                    const ImGuiStyle& s        = ImGui::GetStyle();
                    const float       btnXW =
                        ImGui::CalcTextSize(Orange::Editor::Theme::Icon::GetClose()).x + s.FramePadding.x * 2.0f;
                    ...
                    if (changed && canWrite)
                    {
                        ...
                        if (prop.attribs.assetKind == AssetKind::Material)
                        {
                            for (const auto& p : newVal)
                            {
                                if (!p.empty())
                                {
                                    (void)::EnsureMaterialInstance(host, p);
                                }
                            }
                        }
                ...
                case PropertyType::PolygonVertices:
                {
                    ...
                    const float       closeBtnW =
                        ImGui::CalcTextSize(Orange::Editor::Theme::Icon::GetClose()).x + style.FramePadding.x * 2.0f;
                                    ...
                                    const ComponentSchema& schema)
    {
        ...
        const bool canPaste = host.componentClipboard.valid && schema.typeName != nullptr && host.componentClipboard.typeName == schema.typeName;
        ...
        if (requestPaste && canPaste)
        {
            ...
            {
                auto* pW = pH->scene.pWorld.get();
                if (pW == nullptr || pSchema->get == nullptr)
                {
                    return;
                }
                if (!pW->IsValid(capE))
        ...
        if (open)
        {
            ...
            if (!pluginTakesOver)
            {
                ...
                {
                    if (p.attribs.visibleIf != nullptr && !p.attribs.visibleIf(component))
                    {
                        continue;
                    }
                    if (p.get == nullptr)
                    {
                        continue;
                    } // Group-only / pure header
                    if (p.set == nullptr && !p.attribs.readOnly)
                    {
                        continue;
                    }
                    if (p.label != nullptr)
                ...
                for (const auto& prop : schema.properties)
                {
                    if (prop.attribs.visibleIf != nullptr && !prop.attribs.visibleIf(component))
                    {
                        continue;
                    }
                    ...
                    const bool hasGet = (prop.get != nullptr) || (prop.assetRefGet != nullptr);
                    const bool hasSet = (prop.set != nullptr) || (prop.assetRefSet != nullptr);
                    if (!hasGet)
                    {
                        continue;
                    }
                    if (!hasSet && !prop.attribs.readOnly)
                    {
                        continue;
                    }
            ...
            if (schema.add != nullptr && schema.get != nullptr)
            {
                ...
                host.cmdStack.Push(std::make_unique<LambdaCommand>(
                    ...
                    [pH, entity, removeFn]()
                    {
                        if (pH->scene.pWorld)
                        {
                            removeFn(*pH->scene.pWorld, entity);
                        }
