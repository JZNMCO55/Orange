...
const char* UniformTypeToString(MaterialUniformType type) noexcept
{
    switch (type)
    {
        case MaterialUniformType::Float: return "float";
        case MaterialUniformType::Int:   return "int";
        case MaterialUniformType::Vec2:  return "vec2";
        case MaterialUniformType::Vec3:  return "vec3";
        case MaterialUniformType::Vec4:  return "vec4";
        case MaterialUniformType::Mat4:  return "mat4";
    }
    return "float";
}
...
std::optional<MaterialFileData> ReadMaterialFile(const std::string& path)
{
    ...
    for (std::size_t i = 0; i < uniformCount; ++i)
    {
        ...
        switch (u.type)
        {
            ...
            {
                double tmp = 0.0;
                if (!reader.ReadFloat(valuePath, tmp)) { continue; }
            ...
            {
                std::int64_t tmp = 0;
                if (!reader.ReadInt(valuePath, tmp)) { continue; }
            ...
            {
                float buf[2]{};
                if (!reader.ReadFloatArray(valuePath, buf, 2)) { continue; }
            ...
            {
                float buf[3]{};
                if (!reader.ReadFloatArray(valuePath, buf, 3)) { continue; }
            ...
            {
                float buf[4]{};
                if (!reader.ReadFloatArray(valuePath, buf, 4)) { continue; }
            ...
            {
                float buf[16]{};
                if (!reader.ReadFloatArray(valuePath, buf, 16)) { continue; }
                glm::mat4 m{};
    ...
    const ::Orange::Engine::Asset::AssetRegistry* pAssetRegistry)
{
    ...
    for (const auto& name : names)
    {
        ...
        if (!typeOpt.has_value()) { continue; }  // 不应发生，防御性跳过
        UniformOverrideValue u;
        u.name = name;
        u.type = *typeOpt;
        switch (u.type)
