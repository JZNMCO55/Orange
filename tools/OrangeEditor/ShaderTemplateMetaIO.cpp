...
namespace Orange::Editor::ShaderMeta
{
    namespace
    {
        ...
        UniformWidget ParseWidget(std::string_view s)
        {
            if (s == "hidden")
            {
                return UniformWidget::Hidden;
            }
            if (s == "color")
            {
                return UniformWidget::Color;
            }
            if (s == "slider")
            {
                return UniformWidget::Slider;
            }
            if (s == "drag")
            {
                return UniformWidget::Drag;
            }
            if (s == "components")
            {
                return UniformWidget::Components;
            }
        ...
        {
            double v = 0.0;
            if (!reader.ReadFloat(base + "/step", v))
            {
                return std::nullopt;
            }
        ...
        const std::filesystem::path& jsonPath)
    {
        ...
        if (!reader.ReadString("templateName", meta.templateName) || meta.templateName.empty())
        {
