...
namespace Orange::Editor::Plugin
{
    ...
    namespace
    {
        ...
        const ShaderMeta::ShaderTemplateMeta* FindMeta(std::string_view templateName)
        {
            ...
            {
                if (m.templateName == templateName)
                {
                    return &m;
                }
            }
        ...
        void DrawMaterialSubMode(EditorHost& host, const std::string& materialPath)
        {
            ...
            for (const auto& n : templateNames)
            {
                templateNameCStrs.push_back(n.c_str());
            }
            ...
            if (Orange::Editor::Widgets::BeginPropertyTable("##matprops", 100.0f))
            {
                ...
                if (!templateNameCStrs.empty() && ImGui::Combo("##template", &curTemplateIdx,
                                                               templateNameCStrs.data(),
                                                               static_cast<int>(templateNameCStrs.size())))
                {
                    if (curTemplateIdx >= 0 && curTemplateIdx < static_cast<int>(templateNames.size()))
                    {
    ...
    bool SaveEditingMaterialToDisk(EditorHost& host)
    {
        ...
        if (host.thumbnails)
        {
            host.thumbnails->Invalidate(path);
        }
        return true;
    }
