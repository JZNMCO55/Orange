...
namespace Orange::Editor::Plugin
{
    ...
    namespace
    {
        ...
        {
            return ImGui::CalcTextSize("XXXXXXXXXXXX").x + ImGui::GetStyle().FramePadding.x * 2.0f;
        }
    ...
    {
        if (mEditingPath.empty() || !mEditingValid)
        {
            return;
        }
    ...
    void AnimFsmAssetInspectorPlugin::DrawCanvas(EditorHost& host)
    {
        ...
        for (const auto& t : mEditingFsm.transitions)
        {
            ...
            if (fromIt == mEditingFsm.states.end() || toIt == mEditingFsm.states.end())
            {
    ...
    {
        if (!ImGui::BeginPopup(kAddStatePopupId))
        {
            return;
        }
        ...
        const bool        nameClash =
            nameValid && std::find_if(mEditingFsm.states.begin(), mEditingFsm.states.end(),
                                      [&trimmedName](const ::Orange::Editor::AnimFsm::EditableState& s)
                                      { return s.name == trimmedName; }) != mEditingFsm.states.end();
    ...
    {
        if (!ImGui::BeginPopup(kNodePopupId))
        {
            return;
        }
        ...
        const bool        nameClash =
            newValid && !sameName && std::find_if(mEditingFsm.states.begin(), mEditingFsm.states.end(), [&newName](const ::Orange::Editor::AnimFsm::EditableState& s)
                                                  { return s.name == newName; }) != mEditingFsm.states.end();
        ...
        ImGui::TextDisabled("Transitions from this state:");
        {
            ...
            for (std::size_t i = 0; i < mEditingFsm.transitions.size(); ++i)
            {
                ...
                if (tr.fromState != mRenameTargetState)
                {
                    continue;
                }
                anyShown = true;
    ...
    namespace
    {
                                ...
                                const std::string&                    widgetIdPrefix)
        {
            ...
            const float comboWidth = ImGui::CalcTextSize("XXXXXXXXXX").x + ImGui::GetStyle().FramePadding.x * 2.0f;
            ...
            ImGui::SetNextItemWidth(ImGui::CalcTextSize("XXXX").x + ImGui::GetStyle().FramePadding.x * 2.0f);
            ...
            ImGui::SameLine();
            const float thWidth = ImGui::CalcTextSize("XXXXXXXX").x + ImGui::GetStyle().FramePadding.x * 2.0f;
    ...
    {
        if (!ImGui::BeginPopup("##anim_fsm_add_param"))
        {
            return;
        }
        ...
        const float popupInputW = ImGui::CalcTextSize("XXXXXXXXXXXX").x + ImGui::GetStyle().FramePadding.x * 2.0f;
    ...
    void AnimFsmAssetInspectorPlugin::DrawSelectedTransitionSection(EditorHost& host)
    {
        ...
        const std::string header = "Selected transition: " + t.fromState + " -> " + t.toState + "##sel_tr";
