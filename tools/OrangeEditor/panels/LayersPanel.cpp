...
void EditorRenderLayer::DrawLayersPanel()
{
    ...
    if (ImGui::BeginPopup("##add_layer_popup"))
    {
        ...
        const float inputWidth = ImGui::CalcTextSize("e.g. background").x + ImGui::GetStyle().FramePadding.x * 6.0f;
        ...
        if (ImGui::Button("Create"))
        {
            ...
            mHost.cmdStack.Push(std::make_unique<LambdaCommand>(
                "add_layer",
                [pH, info]()
                { pH->scene.partition.AddLayer(info); },
                [pH, idStr = info.id]()
                { pH->scene.partition.RemoveLayer(idStr); }));
    ...
    for (const auto& layer : layers)
    {
        ...
        else
        {
            ...
            if (canEdit && ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
            {
            ...
            if (canEdit && ImGui::BeginDragDropTarget())
            {
                ...
                {
                    if (pl->Data != nullptr && pl->DataSize == static_cast<int>(sizeof(Orange::Engine::Entity)))
                    {
        ...
        const float clusterW  = ImGui::CalcTextSize(upIcon).x + pad + ImGui::CalcTextSize(downIcon).x + pad + ImGui::CalcTextSize(closeIcon).x + pad + spacing * 2.0f;
    ...
    if (!pendingMoveId.empty() && pendingMoveDelta != 0)
    {
        ...
        mHost.cmdStack.Push(std::make_unique<LambdaCommand>(
            "move_layer",
            [pH, idStr, delta]()
            { pH->scene.partition.MoveLayer(idStr, delta); },
            [pH, idStr, delta]()
            { pH->scene.partition.MoveLayer(idStr, -delta); }));
    }
