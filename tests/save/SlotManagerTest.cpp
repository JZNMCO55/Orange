...
SaveGameRegistry MakeRegistry()
{
    ...
    auto rc = reg.Register<PlayerInventory>(
        ...
        {
            std::int64_t v = 0;
            if (!r.ReadInt(std::string(path) + "/goldCoins", v)) { return false; }
            c.goldCoins = v;
            return true;
        });
    assert(rc.IsOk());
...
void TestListSlotsMultipleAndDegradation()
{
    ...
    SlotMetadata m1{}; m1.savedAtUnixSeconds = 1000; m1.displayName = "First";
    SlotMetadata m2{}; m2.savedAtUnixSeconds = 3000; m2.displayName = "Third";
    SlotMetadata m3{}; m3.savedAtUnixSeconds = 2000; m3.displayName = "Second";
    ...
    {
        if (m.slotName == "slotA") { seenA = true; assert(m.displayName == "First"); }
        if (m.slotName == "slotB") { seenB = true; assert(m.displayName == "Third"); }
        if (m.slotName == "slotC")
        {
            seenC = true;
    ...
    {
        if (list[i].slotName == "slotA") { posA = i; }
        if (list[i].slotName == "slotB") { posB = i; }
    }
    assert(posB < posA);
}
