...
SaveGameRegistry MakeRegistry()
{
    ...
    auto rc2 = reg.Register<CheckpointTag>(
        ...
        {
            std::int64_t v = 0;
            if (!r.ReadInt(std::string(path) + "/checkpointId", v)) { return false; }
            c.checkpointId = v;
            return true;
        });
    assert(rc2.IsOk());
...
void TestSingleEntitySingleComponent()
{
    ...
    if (view.begin() != view.end()) { restored = World::FromEntt(*view.begin()); }
...
void TestMultipleEntitiesAndComponents()
{
    ...
    {
        if (p.first == 100 && p.second == 3) { found100 = true; }
        if (p.first == 0   && p.second == 99) { found0 = true; }
    }
