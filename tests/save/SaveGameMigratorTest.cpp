...
namespace
{
    ...
    SaveGameRegistry::ReadFn<PlayerInventory> ReadV3()
    {
        return [](const JsonReader& r, std::string_view path, PlayerInventory& c) -> bool
        {
            ...
            std::int64_t      v = 0;
            if (!r.ReadInt(b + "/goldCoins", v))
            {
                return false;
            }
            c.goldCoins = v;
            if (!r.ReadInt(b + "/spiritShards", v))
            {
                return false;
            }
            c.spiritShards = v;
            if (!r.ReadInt(b + "/maxHp", v))
            {
                return false;
            }
            c.maxHp = v;
    ...
    auto MigratorV2ToV3()
    {
                  ...
                  JsonWriter& dst, std::string_view dstPath) -> bool
        {
            ...
            dst.WriteInt(db + "/goldCoins", v);
            if (!src.ReadInt(sb + "/spiritShards", v))
            {
                return false;
            }
    ...
    {
        return [](const JsonReader&, std::string_view, PlayerInventory&) -> bool
        { return true; };
    }
    ...
    std::filesystem::path WriteV1Save(const char* tag, std::int64_t goldCoins)
    {
        ...
        auto             rc = writerReg.Register<PlayerInventory>(
            ...
            [](const JsonReader&, std::string_view, PlayerInventory&)
            { return true; });
        assert(rc.IsOk());
