...
const EaseType kAllTypes[] = {
    EaseType::Linear,
    EaseType::InSine,   EaseType::OutSine,   EaseType::InOutSine,
    EaseType::InQuad,   EaseType::OutQuad,   EaseType::InOutQuad,
    ...
    EaseType::InExpo,   EaseType::OutExpo,   EaseType::InOutExpo,
    EaseType::InCirc,   EaseType::OutCirc,   EaseType::InOutCirc,
...
const EaseType kInOutTypes[] = {
    EaseType::InOutSine,  EaseType::InOutQuad,    EaseType::InOutCubic,
    EaseType::InOutQuart, EaseType::InOutQuint,   EaseType::InOutExpo,
    EaseType::InOutCirc,  EaseType::InOutBack,    EaseType::InOutElastic,
...
int main()
{
    ...
    {
        const EaseType reps[] = {
            EaseType::Linear,   EaseType::InQuad,   EaseType::OutCubic,
            EaseType::InOutExpo,EaseType::InBack,   EaseType::OutElastic,
            EaseType::InBounce,
        };
    ...
    // ===== 7. Tween：Once 线性 from=0,to=10,duration=2 —— 半程 5 + 完成 10 + onComplete 一次 =====
    {
        ...
        desc.onUpdate   = [&lastValue](float v) { lastValue = v; };
        desc.onComplete = [&completeCount]() { ++completeCount; };
    ...
    // ===== 11. Tween：onUpdate 每次 Update 被调、值正确 =====
    {
        ...
        desc.duration = 4.0f;
        desc.ease     = EaseType::Linear;
        desc.onUpdate = [&values](float v) { values.push_back(v); };
    ...
    // ===== 16. 瞬间（duration<=0）+ 负 duration 补间完成语义（对抗式复核逮到边界 bug 修复回归）=====
    {
        // duration=0 无 delay：首帧 Update 即完成，观测值必须是 to（非 from），onComplete 一次。
        {
            ...
            Tween::TweenDesc d;
            d.from = 5.0f; d.to = 10.0f; d.duration = 0.0f; d.delay = 0.0f;
            d.loop       = Tween::LoopMode::Once;
            d.onComplete = [&] { ++completeCount; };
            d.onUpdate   = [&](float v) { lastUpdate = v; };
        ...
        // duration=0 + delay=0.5：先走 delay（值=from），delay 后瞬间完成到 to。
        {
            ...
            int              completeCount = 0;
            Tween::TweenDesc d;
            d.from = 0.0f; d.to = 100.0f; d.duration = 0.0f; d.delay = 0.5f;
            d.loop       = Tween::LoopMode::Once;
            d.onComplete = [&] { ++completeCount; };
            const Tween::TweenHandle h = mgr.Add(d);
        ...
        // 负 duration + delay：在 delay 内（pt<0）绝不能提前完成。
        {
            ...
            int              completeCount = 0;
            Tween::TweenDesc d;
            d.from = 0.0f; d.to = 1.0f; d.duration = -1.0f; d.delay = 5.0f;
            d.loop       = Tween::LoopMode::Once;
            d.onComplete = [&] { ++completeCount; };
            const Tween::TweenHandle h = mgr.Add(d);
    ...
    // ===== 17. Loop / PingPong 永不触发 onComplete（契约反证） =====
    {
        ...
        int              pingComplete = 0;
        Tween::TweenDesc dl;
        dl.from = 0.0f; dl.to = 1.0f; dl.duration = 1.0f; dl.loop = Tween::LoopMode::Loop;
        dl.onComplete = [&] { ++loopComplete; };
        Tween::TweenDesc dp = dl;
        dp.loop       = Tween::LoopMode::PingPong;
        dp.onComplete = [&] { ++pingComplete; };
        const Tween::TweenHandle hl = mgr.Add(dl);
        const Tween::TweenHandle hp = mgr.Add(dp);
        for (int i = 0; i < 10; ++i) { mgr.Update(0.7f); }  // 跨多个周期
    ...
    {
        struct EaseRef { Tween::EaseType type; float v25; float v5; float v75; };
        const EaseRef refs[] = {
