...
std::vector<std::uint8_t> MakeTinyWav()
{
    ...
    auto u16 = [&](std::uint16_t v) { b.push_back(v & 0xFF); b.push_back((v >> 8) & 0xFF); };
    auto u32 = [&](std::uint32_t v) {
        b.push_back(v & 0xFF);          b.push_back((v >> 8) & 0xFF);
        b.push_back((v >> 16) & 0xFF);  b.push_back((v >> 24) & 0xFF);
    };
    auto tag = [&](const char* s) {
        for (int i = 0; i < 4; ++i) { b.push_back(static_cast<std::uint8_t>(s[i])); }
    };
    const std::uint32_t dataBytes = 8;  // 4 样本 * 2 字节
    tag("RIFF"); u32(36 + dataBytes); tag("WAVE");
    tag("fmt "); u32(16); u16(1); u16(1); u32(8000); u32(16000); u16(2); u16(16);
    tag("data"); u32(dataBytes);
    for (int i = 0; i < 4; ++i) { u16(0); }  // 4 个静音样本
    return b;
}
...
void TestSoundInstanceMoveAssignReleasesOldSound()
{
    ...
    desc.useNullBackend = true;
    Au::AudioEngine engine(desc);
    if (!engine.IsInitialized()) { return; }  // null backend 应成功
