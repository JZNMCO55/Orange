...
namespace
{
                       ...
                       float dt, float dv)
    {
        ...
        if (dt > 1e-6f)
        {
            t.x = std::fmin(std::fmax((handleTime - k0.time) / dt, 0.0f), 1.0f);
        }
        if (std::fabs(dv) > 1e-6f)
        {
            t.y = (handleValue - k0.value.x) / dv;
        }
        return t;
    }
                      ...
                      float dt, float dv)
    {
        ...
        if (dt > 1e-6f)
        {
            t.x = std::fmin(std::fmax((handleTime - k1.time) / dt, -1.0f), 0.0f);
        }
        if (std::fabs(dv) > 1e-6f)
        {
            t.y = (handleValue - k1.value.x) / dv;
        }
        return t;
    }
...
int main()
{
    ...
    // ===== 5. 多 key：编辑中间 key 的切线只影响相邻两段 =====
    {
        ...
        assert(Near(seg2MidBefore, 50.0f) && Near(seg2MidAfter, 50.0f) && "平直段（端点同值）中点恒为该值，切线不改值");
