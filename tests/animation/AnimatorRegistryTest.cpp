...
namespace
{
    ...
    void TestRegisterDuplicateRejected()
    {
        ...
        int                    v1 = 0;
        int                    v2 = 0;
        reg.RegisterBackend("dup", [&]
                            { v1 = 1; return std::unique_ptr<EchoAnimator>(new EchoAnimator); });
        auto rc = reg.RegisterBackend("dup", [&]
                                      { v2 = 1; return std::unique_ptr<EchoAnimator>(new EchoAnimator); });
        assert(rc.IsErr());
    ...
    {
        Anim::AnimatorRegistry reg;
        reg.RegisterBackend("ghost", []
                            { return std::unique_ptr<EchoAnimator>(); });
