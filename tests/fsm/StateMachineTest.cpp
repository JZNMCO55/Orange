...
namespace
{
    ...
    StateCallbacks TracingCallbacks(const std::string& name)
    {
        StateCallbacks cb;
        cb.onEnter = [name]()
        { gTrace.push_back("enter" + name); };
        cb.onExit = [name]()
        { gTrace.push_back("exit" + name); };
        cb.onUpdate = [name](float)
        { gTrace.push_back("update" + name); };
        return cb;
    }
    ...
    void TestGuardedTransitions()
    {
        ...
        bool first  = false;
        bool second = true;
        sm2.AddTransition(SIdle, EGo, SRun, [&]()
                          { return first; }); // 第一条：guard=false
        sm2.AddTransition(SIdle, EGo, SJump, [&]()
                          { return second; }); // 第二条：guard=true
        sm2.Start(SIdle);
        assert(sm2.Fire(EGo));
        ...
        sm3.AddState(SRun);
        sm3.AddState(SJump);
        sm3.AddTransition(SIdle, EGo, SRun, [&]()
                          { return true; });
        sm3.AddTransition(SIdle, EGo, SJump, [&]()
                          { return true; });
        sm3.Start(SIdle);
        assert(sm3.Fire(EGo));
        ...
        sm.AddTransition(SIdle, EGo, SRun, [&sm]()
                         {
                             for (int i = 0; i < 64; ++i)
                             {
                                 sm.AddTransition(SIdle, EJmp, SJump, []()
                                                  { return false; });
                             }
                             return false; // 本条不通过
                         });
