...
void TestBasicPublishSubscribe()
{
    ...
    Ev::SubscriptionHandle h = bus.Subscribe<FooEvent>(
        [&received](const FooEvent& e) { received = e.value; });
    assert(h.IsValid());
...
void TestMultipleHandlersOrder()
{
    ...
    bus.Subscribe<FooEvent>([&order](const FooEvent&) { order.push_back(1); });
    bus.Subscribe<FooEvent>([&order](const FooEvent&) { order.push_back(2); });
    bus.Subscribe<FooEvent>([&order](const FooEvent&) { order.push_back(3); });
...
void TestTypeIsolation()
{
    ...
    int fooHits = 0;
    int barHits = 0;
    bus.Subscribe<FooEvent>([&fooHits](const FooEvent&) { ++fooHits; });
    bus.Subscribe<BarEvent>([&barHits](const BarEvent&) { ++barHits; });
...
void TestUnsubscribe()
{
    ...
    Ev::SubscriptionHandle h =
        bus.Subscribe<FooEvent>([&hits](const FooEvent&) { ++hits; });
    assert(bus.HandlerCount<FooEvent>() == 1);
...
void TestEnqueueDispatchQueued()
{
    ...
    bus.Subscribe<FooEvent>(
        [&received](const FooEvent& e) { received.push_back(e.value); });
...
void TestReentrantSafety()
{
    ...
    self = bus.Subscribe<FooEvent>(
        ...
        {
            ++fooA;
            bus.Subscribe<FooEvent>([&fooB](const FooEvent&) { ++fooB; });
...
void TestClear()
{
    ...
    int hits = 0;
    bus.Subscribe<FooEvent>([&hits](const FooEvent&) { ++hits; });
    bus.Subscribe<BarEvent>([&hits](const BarEvent&) { ++hits; });
    bus.Enqueue<FooEvent>(FooEvent{1});
    assert(bus.HandlerCount<FooEvent>() == 1);
