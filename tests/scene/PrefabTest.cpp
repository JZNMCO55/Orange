...
void TestTemplateAnchoring(const fs::path& root)
{
    ...
    assert(templateWorld.GetComponent<GuidComponent>(tmplMatchRoot)->guid
           == rootLink->templateEntityGuid);
    assert(templateWorld.GetComponent<GuidComponent>(tmplMatchChild)->guid
           == childLink->templateEntityGuid);
    ...
    assert(w2.GetComponent<PrefabInstanceComponent>(reloadedRoot)->templateEntityGuid
           == tmplRootGuid);
    assert(w2.GetComponent<PrefabInstanceComponent>(reloadedChild)->templateEntityGuid
           == tmplChildGuid);
