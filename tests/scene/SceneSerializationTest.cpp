...
namespace
{
    ...
    void TestPolygonAndEdgeChainShapesRoundTrip()
    {
        ...
        for (auto ent : reg.view<NameComponent>())
        {
            const auto& nc = reg.get<NameComponent>(ent);
            if (nc.name == "poly")
                polyE = World::FromEntt(ent);
            else if (nc.name == "chain")
                chainE = World::FromEntt(ent);
        }
    ...
    void TestClipAnimatorAssetSourceRoundTrip()
    {
        ...
        const auto animPath  = (std::filesystem::temp_directory_path() /
                               "orange_engine_scene_test_src.anim")
                                  .string();
        ...
        // 先落一个 .anim 资产到盘。
        {
            ...
            Anim::Keyframe k0;
            k0.time  = 0.0f;
            k0.value = glm::vec4(0, 0, 0, 0);
            Anim::Keyframe k1;
            k1.time  = 1.0f;
            k1.value = glm::vec4(6, 0, 0, 0);
            t.keys.push_back(k0);
            t.keys.push_back(k1);
        ...
        Entity loadedE = Entity::Invalid();
        for (auto ent : lreg.view<AnimatorComponent>())
        {
            loadedE = World::FromEntt(ent);
        }
        assert(loadedE.IsValid());
            ...
            out <<
              ...
              "entities": [
                { "id": 0, "components": {
                    ...
                    "Guid": { "value": ")"
                << gP.ToString() << R"(" },
                    "Hierarchy": { "parent": -1, "firstChild": 1, "firstChildGuid": ")"
                << gK.ToString() << R"(",
                ...
                { "id": 1, "components": {
                    ...
                    "Guid": { "value": ")"
                << gK.ToString() << R"(" },
                    "Hierarchy": { "parent": 99, "parentGuid": ")"
                << gP.ToString() << R"(",
