...
namespace
{
    bool Eq(float a, float b, float eps = 1e-5f) noexcept
    {
...
int main()
{
    const auto      path = (std::filesystem::temp_directory_path() / "orange_pp_roundtrip.scene.json").string();
    std::error_code ec;
    ...
    src.ssaoEnabled             = true;
    src.ssaoUseGtao             = false;
    src.ssaoRadius              = 0.42f;
    src.ssaoStrength            = 0.8f;
    src.ssaoPower               = 2.5f;
    src.ssrEnabled              = false;
    src.ssrMaxDistance          = 9.0f;
    src.ssrThickness            = 0.4f;
    src.ssrStrength             = 0.3f;
    src.contactEnabled          = true;
    src.contactLength           = 0.2f;
    src.contactThickness        = 0.25f;
    src.contactStrength         = 0.7f;
    src.dofEnabled              = true;
    src.dofFocusDistance        = 7.5f;
    src.dofFocusRange           = 6.0f;
    src.dofMaxCoCRadius         = 0.02f;
    src.taaEnabled              = true;
    src.taaFeedback             = 0.85f;
    src.gradeEnabled            = true;
    src.gradeExposure           = 0.3f;
    src.gradeContrast           = 1.2f;
    src.gradeSaturation         = 0.9f;
    src.gradeTemperature        = -0.4f;
    src.gradeTint               = 0.1f;
    src.motionBlurEnabled       = true;
    src.motionBlurIntensity     = 0.7f;
    src.motionBlurMaxRadius     = 0.06f;
    src.motionBlurSampleCount   = 12;
    src.lensEnabled             = true;
    src.lensChromaticAberration = 0.005f;
    src.lensVignetteIntensity   = 0.45f;
    src.lensVignetteSmoothness  = 0.6f;
    src.sharpenEnabled          = true;
    src.sharpenStrength         = 0.55f;
    src.pcssLightSize           = 8.0f;
    src.shadowMapResolution     = 4096;
    ...
    { if (!cond) { std::fprintf(stderr, "  字段不匹配: %s\n", what); ok = false; } };
    ...
    chk(Eq(l->gradeExposure, 0.3f) && Eq(l->gradeContrast, 1.2f) && Eq(l->gradeSaturation, 0.9f) && Eq(l->gradeTemperature, -0.4f) && Eq(l->gradeTint, 0.1f), "grade params");
    chk(l->motionBlurEnabled, "motionBlurEnabled");
    chk(Eq(l->motionBlurIntensity, 0.7f) && Eq(l->motionBlurMaxRadius, 0.06f) && l->motionBlurSampleCount == 12, "motion blur params");
    chk(l->lensEnabled, "lensEnabled");
    chk(Eq(l->lensChromaticAberration, 0.005f) && Eq(l->lensVignetteIntensity, 0.45f) && Eq(l->lensVignetteSmoothness, 0.6f), "lens params");
