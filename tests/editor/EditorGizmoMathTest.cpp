...
bool ApproxEq(float a, float b) { return std::fabs(a - b) < kEps; }
...
void TestPointSegmentDistance2D()
{
    ...
    assert(ApproxEq(GM::PointSegmentDistance2D(
        glm::vec2(0, 1), glm::vec2(-1, 0), glm::vec2(1, 0)), 1.0f));
    // 端点 clamp：p 在 b 外侧 → 最近点 = b。
    assert(ApproxEq(GM::PointSegmentDistance2D(
        glm::vec2(2, 0), glm::vec2(-1, 0), glm::vec2(1, 0)), 1.0f));
    // 落在段上 → 0。
    assert(ApproxEq(GM::PointSegmentDistance2D(
        glm::vec2(0, 0), glm::vec2(-1, 0), glm::vec2(1, 0)), 0.0f));
