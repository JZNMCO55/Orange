...
int main()
{
                        ...
                        std::size_t& outResidentAfter) -> bool
    {
        ...
        // 5) readback 中心像素。
        float      px[4]  = {0, 0, 0, 0};
        const bool readOk = renderOk && ReadbackCenter(*pDevice, *target, kThumbW, kThumbH, px);
    ...
    // ---- case 1：居中 scene → AABB 框相机 → 中心非黑 + 清理无残留 -----------
    {
        ...
        assert(lum > 0.05f && "居中 scene 缩略图中心像素全黑 —— 场景未被框定 / 渲染到 target");
        // 关键：多根清理后 scratch 必须只剩常驻 camera + light（created 全销毁）。
        assert(residentAfter == kResidentCount && "case1 多根清理后 scratch 有残留 —— created 未清干净");
        std::fprintf(stdout,
    ...
    // 断言非黑 = AABB 框定真把相机平移跟随了。复用同一 scratch，再验一次清理无残留。
    {
        ...
        std::fprintf(stderr,
                     ...
                     lum, residentAfter, ok ? 1 : 0);
        assert(ok && lum > 0.05f && "偏离原点 scene 缩略图中心全黑 —— AABB 框定未跟随场景位置");
        // 第二次烘焙复用同一 scratch，清理后仍只剩常驻实体（验证复用不累积污染）。
        assert(residentAfter == kResidentCount && "case2 多根清理后 scratch 有残留 —— 复用 scratch 累积了污染");
        std::fprintf(stdout,
